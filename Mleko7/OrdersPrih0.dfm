inherited fmOrdersPrih: TfmOrdersPrih
  Left = 759
  Top = 143
  ...
  object quPlatP: TMSQuery
    ...
    SQL.Strings = (
        ...
        'select PlatP.PlatNo,PlatP.Nom,PlatP.PostNo,PlatP.Summa,PlatP.Dat' +
        'ePlat,'
      '       PlatP.Spravka,PlatP.Buh,Post.Name,f.num as  kass_num '
      'from PlatP join Post on (Post.PostNo=PlatP.PostNo)'
      
        'left outer join d_fiscal f on upper(f.HostName)=upper(PlatP.Host' +
        'NAme)'
      'where &_where '
      'order by &_order')
    CachedUpdates = True
    UpdateObject = UpdateSQL1
    BeforePost = quPlatPBeforePost
    BeforeDelete = quPlatPBeforeDelete
    ...
    object quPlatPkass_num: TIntegerField
      FieldName = 'kass_num'
      Origin = 'd_fiscal.num'
    end
  end

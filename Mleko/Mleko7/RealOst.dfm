inherited fmRealOst: TfmRealOst
  Left = 216
  Top = 240
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'fmRealOst'
  ClientHeight = 426
  ClientWidth = 642
  OldCreateOrder = True
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object CoolBar1: TCoolBar [0]
    Left = 0
    Top = 0
    Width = 642
    Height = 31
    Bands = <>
    EdgeOuter = esNone
    object sbInsert: TSpeedButton
      Left = 4
      Top = 0
      Width = 29
      Height = 29
      Hint = #1053#1086#1074#1099#1081' '#1089#1086#1090#1088#1091#1076#1085#1080#1082
      Glyph.Data = {
        
        2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C2C}
      ParentShowHint = False
      ShowHint = True
      OnClick = sbInsertClick
    end
    object SpeedButton4: TSpeedButton
      Left = 38
      Top = 0
      Width = 29
      Height = 29
      Hint = #1055#1077#1095#1072#1090#1100
      Glyph.Data = {
        
        AFAFAFAFAFAFAFAFAFAF74704EAFAFAFAFAFAFAFAFAFAFAFAFAF}
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton4Click
    end
  end
  object Panel1: TPanel [1]
    Left = 0
    Top = 385
    Width = 642
    Height = 41
    Align = alBottom
    TabOrder = 2
    object bbOk: TBitBtn
      Left = 277
      Top = 8
      Width = 81
      Height = 25
      Caption = #1054#1050
      ModalResult = 1
      TabOrder = 0
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
  end
  object dbgRealOst: TDBGridEh [2]
    Left = 0
    Top = 31
    Width = 642
    Height = 354
    Align = alClient
    DataSource = dsRealOst
    Flat = True
    FooterColor = clWindow
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -11
    FooterFont.Name = 'MS Sans Serif'
    FooterFont.Style = []
    Options = [dgEditing, dgTitles, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDrawColumnCell = RxDBGrid1DrawColumnCell
    OnKeyDown = RxDBGrid1KeyDown
    OnKeyPress = RxDBGrid1KeyPress
    OnTitleBtnClick = RxDBGrid1TitleBtnClick
    Columns = <
      item
        EditButtons = <>
        FieldName = 'NameTovarShort'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
        Title.TitleButton = True
        Width = 113
      end
      item
        EditButtons = <>
        FieldName = 'NameTovar'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #1055#1086#1083#1085#1086#1077' '#1085#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
        Title.TitleButton = True
        Width = 201
      end
      item
        EditButtons = <>
        FieldName = 'VidName'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #1042#1080#1076
        Title.TitleButton = True
        Width = 112
      end
      item
        EditButtons = <>
        FieldName = 'Kol'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #1050#1086#1083'-'#1074#1086
        Title.TitleButton = True
        Width = 60
      end
      item
        EditButtons = <>
        FieldName = 'kol_ost'
        Footers = <>
        Title.Caption = #1050#1086#1083'. '#1069#1083#1077#1082#1090
        Width = 60
      end
      item
        EditButtons = <>
        FieldName = 'kol_calc'
        Footers = <>
        Title.Caption = #1050#1086#1083'. '#1056#1072#1089#1095'.'
        Width = 60
      end>
  end
  object dsRealOst: TDataSource
    DataSet = quRealOst
    Left = 24
    Top = 76
  end
  object quOstatok: TMSQuery
    Connection = dmDataModule.DB
    SQL.Strings = (
      
        'select cr.TovarNo,a.NameTovar,a.NameTovarShort, r.kol ,cr.qty as' +
        ' calc_qty'
      
        'from tovar a left outer join dbo.v_calc_rest cr on a.TovarNo=cr.' +
        'TovarNo'
      '             left outer join ostatok r on a.TovarNo=r.TovarNo'
      'where cr.qty<>0 or r.kol<>0'
      'ORDER BY a.TovarNo')
    Left = 24
    Top = 122
    object quOstatokTovarNo: TSmallintField
      FieldName = 'TovarNo'
    end
    object quOstatokNameTovar: TStringField
      FieldName = 'NameTovar'
      Size = 30
    end
    object quOstatokNameTovarShort: TStringField
      FieldName = 'NameTovarShort'
      Size = 14
    end
    object quOstatokcalc_qty: TFloatField
      FieldName = 'calc_qty'
    end
    object quOstatokkol: TFloatField
      FieldName = 'kol'
    end
  end
  object quWork: TMSQuery
    Connection = dmDataModule.DB
    Left = 78
    Top = 123
  end
  object quRealOst: TMSQuery
    Connection = dmDataModule.DB
    SQL.Strings = (
      
        'SELECT Tovar.TovarNo, Tovar.NameTovar, Tovar.NameTovarShort, Vid' +
        'Tov.VidName, OstatokReal.Kol , OstatokReal.DateWork,kol_ost, kol' +
        '_calc'
      'FROM         OstatokReal INNER JOIN'
      
        '                      Tovar ON OstatokReal.TovarNo = Tovar.Tovar' +
        'No INNER JOIN'
      '                      VidTov ON Tovar.VidNo = VidTov.VidNo'
      
        'WHERE (OstatokReal.DateWork = :DateWork) AND (Tovar.OtdelNo <> 3' +
        ') '
      'ORDER BY VidTov.VidName, Tovar.NameTovar')
    Left = 130
    Top = 123
    ParamData = <
      item
        DataType = ftDateTime
        Name = 'DateWork'
        Value = 38429d
      end>
    object quRealOstTovarNo: TSmallintField
      FieldName = 'TovarNo'
      ReadOnly = True
    end
    object quRealOstKol: TFloatField
      FieldName = 'Kol'
      Required = True
    end
    object quRealOstNameTovar: TStringField
      FieldName = 'NameTovar'
      ReadOnly = True
      Size = 30
    end
    object quRealOstNameTovarShort: TStringField
      FieldName = 'NameTovarShort'
      ReadOnly = True
      Size = 14
    end
    object quRealOstVidName: TStringField
      FieldName = 'VidName'
      ReadOnly = True
      Size = 30
    end
    object quRealOstkol_ost: TFloatField
      FieldName = 'kol_ost'
    end
    object quRealOstkol_calc: TFloatField
      FieldName = 'kol_calc'
    end
  end
  object frReport8: TfrReport
    InitialZoom = pzDefault
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit]
    RebuildPrinter = False
    Left = 296
    Top = 132
    ReportForm = {19000000}
  end
  object frDBDataSet1: TfrDBDataSet
    DataSource = dsRealOst
    Left = 228
    Top = 128
  end
end

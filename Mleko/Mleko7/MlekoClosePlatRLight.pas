...
uses
  ...
  StdCtrls, comp_Target_DBCtrls, VirtualTable, Math,
...
type
  TMlekoClosePlatRLightForm = class(TCFLMLKListForm)
    ...
    procedure DSLinkSummaDataChange(Sender: TObject; Field: TField);
...
var
  ...
  k: integer;
  MlekoClosePlatRLightForm: TMlekoClosePlatRLightForm;
  StrOtdFlt: string;
...
procedure TMlekoClosePlatRLightForm.SetActionsEnabled;
var
...
procedure TMlekoClosePlatRLightForm.ActionGoExcelExecute(Sender: TObject);
begin
  inherited;
  DbGrid.CopyGridToExcel;
end;

procedure TMlekoClosePlatRLightForm.ActionShowLinkExecute(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.SetLinkPanelVisible(aIsVisible: boolean);
var
  i: integer;
begin
  if aIsVisible then
...
procedure TMlekoClosePlatRLightForm.ActionLinkRefreshExecute(Sender: TObject);
begin
  inherited;
  DBGridPaymentLink.Refresh;
end;

function TMlekoClosePlatRLightForm.DBGridGetKey(Sender: TObject;
  DataSet: TDataSet): string;
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.QueryAfterScroll(DataSet: TDataSet);
begin
  inherited;
  ActionScrollExecute(Self);
end;

procedure TMlekoClosePlatRLightForm.ActionLinkLayoutExecute(Sender: TObject);
begin
  inherited;
...
function TMlekoClosePlatRLightForm.FilterFirma_OnSelectValue(
  Sender: TTargetFilter_DictionaryFilter; BtnBoundsRect: TRect;
  ConditionType: TDictionaryConditionType;
  ConditionGroup: TDictionaryConditionGroup; var Rec_PK,
  Rec_Title: Variant): Boolean;
...
procedure TMlekoClosePlatRLightForm.ActionViewRefreshExecute(Sender: TObject);
var
...
procedure TMlekoClosePlatRLightForm.ActionLinkDelExecute(Sender: TObject);
var
  i: integer;
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      spAddPlatToNakl.Params.ParamByName('p_Link_SummaCurrencyAccounting').Value := 0;
...
procedure TMlekoClosePlatRLightForm.GoExelLinkExecute(Sender: TObject);
begin
  inherited;
  DbGridPaymentLink.CopyGridToExcel;
end;

procedure TMlekoClosePlatRLightForm.ActionShowFreeNaklExecute(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.SetFreeNaklPanelVisible(aIsVisible: boolean);
var
  i: integer;
...
//    ShowMessage(Query.SQL.Text);
    ...
    str := str + ' ,cast(0 as decimal(18,6)) as BeforeSummaAccauntCurrency ';
...
procedure TMlekoClosePlatRLightForm.ActionFreeNaklRefreshExecute(
  Sender: TObject);
var
  str: string;
begin
...
procedure TMlekoClosePlatRLightForm.ActionFreeNaklChLayotExecute(
  Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ActionItemPropExecute(Sender: TObject);
var
  ln_id: Int64;
  Params: TParams;
begin
...
procedure TMlekoClosePlatRLightForm.ActionItemDelExecute(Sender: TObject);
var
i : integer;
table_key : Double;
    ...
    begin
      ...
      if DBGrid.MarkItems.Count > 0 then
       for i:=0 to DBGrid.MarkItems.Count-1 do
        begin
          spDelEntity.Close;
          spDelEntity.Params.ParamByName('Pkey').Value := DBGrid.MarkItems[i];
...
function TMlekoClosePlatRLightForm.DBGridFreeNaklGetKey(Sender: TObject;
  DataSet: TDataSet): string;
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.QueryAfterRefresh(DataSet: TDataSet);
begin
  inherited;
  ActionScrollExecute(Self);
end;

procedure TMlekoClosePlatRLightForm.QueryAfterOpen(DataSet: TDataSet);
begin
  inherited;
  ActionScrollExecute(Self);
end;

procedure TMlekoClosePlatRLightForm.ActionLinkPlatExecute(Sender: TObject);
var
  i: integer;
  l_rest_summa: real;
  l_close_summa: real;
...
//  showmessage (Query.FieldByName('FreeSumma').AsString);
  ...
  if Query.FieldByName('Rate').IsNull then begin
                                      ...
                                      else Rate := Query.FieldByName('Rate').AsFloat;

  if Query.FieldByName('RateCurrencyAccounting').IsNull then begin
                                                               dmDataModule.OpenSQL('select ce.Rate from D_CURRENCY c inner join CurrencyExchange ce on c.IsDefault = 1 and ce.IsActive = 1 and ce.CurrencyId = c.ID and c.isTrash = 0');
                                                               RateCurrencyAccounting := dmDataModule.qfo.FieldByName('Rate').Value;
                                                             end
                                                        else RateCurrencyAccounting := Query.FieldByName('RateCurrencyAccounting').AsFloat;
...
//      DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;

      l_rest_summa := abs(DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat);

      l_rest_summaCurrencyAccounting := RoundTo (l_rest_summa * Rate / RateCurrencyAccounting,-2);
        ...
        if l_rest_summa < abs(QuMrkNkl.FieldByName('FreeSumma').AsFloat) then
         begin
          ...
          l_close_summa_CurrencyAccounting := l_rest_summaCurrencyAccounting;
               ...
               l_close_summa_CurrencyAccounting := abs(QuMrkNkl.FieldByName('SummaDolgCurrencyAccounting').AsFloat); // * Rate / RateCurrencyAccounting;
             end;
{
...
procedure TMlekoClosePlatRLightForm.ftOtdelSelectOk(Sender: TObject);
begin
  inherited;
  //  QuFreeNakl.Params.ParamByName('OtdelNo').Value:=StrToInt(ftOtdel.KeyFieldValue);
end;

procedure TMlekoClosePlatRLightForm.DBGridEh1DrawFooterCell(Sender: TObject;
  DataCol, Row: Integer; Column: TColumnEh; Rect: TRect;
  State: TGridDrawState);
begin
  inherited;
//  if DSLinkSumma.DataSet.State in [dsInsert, dsEdit] then DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;
//  DBGridEh1.FieldColumns['BeforeSummaAccauntCurrency'].DisplayText := DBGridEh1.FieldColumns['BeforeSumma'].DisplayText;
...
procedure TMlekoClosePlatRLightForm.DBGridEh1GetCellParams(Sender: TObject;
...
{
  if Column.FieldName = 'BeforeSumma' then begin
//                                             if DSLinkSumma.DataSet.State <> dsEdit then DSLinkSumma.DataSet.Edit;
                                             if DSLinkSumma.DataSet.State in [dsInsert,dsEdit] then begin
                                                                                                      DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;
                                                                                                      DSLinkSumma.DataSet.Post;
                                                                                                      DSLinkSumma.DataSet.Refresh;
                                                                                                    end;  
                                           end;
}                                           
end;

procedure TMlekoClosePlatRLightForm.ActionMarkLinkRefreshExecute(
  Sender: TObject);
var
...
function TMlekoClosePlatRLightForm.SQLBuilderFreeNaklGetWhere(
...
procedure TMlekoClosePlatRLightForm.SQLBuilderLinkGetParams(Sender: TObject;
  var Params: TParams);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ActionScrollExecute(Sender: TObject);
var
  Str: string;
  ln_key: Integer;
begin
...
procedure TMlekoClosePlatRLightForm.SQLBuilderGetParams(Sender: TObject;
  var Params: TParams);
var
...
function TMlekoClosePlatRLightForm.DBGridPaymentLinkGetKey(Sender: TObject;
  DataSet: TDataSet): string;
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ActionItemAddExecute(Sender: TObject);
var
  ln_id: Int64;
  Params: TParams;
begin
...
procedure TMlekoClosePlatRLightForm.ActionRecalcSumExecute(Sender: TObject);
var
  i: integer;
begin
  inherited;
...
function TMlekoClosePlatRLightForm.DBGridIsLighted(Sender: TObject;
  DataSet: TDataSet): Boolean;
begin
  inherited;
  result := False;
...
function TMlekoClosePlatRLightForm.DBGridFreeNaklIsLighted(Sender: TObject;
  DataSet: TDataSet): Boolean;
begin
  inherited;
  result := (abs(DataSet.FieldByName('Summa').AsFloat - DataSet.FieldByName('CloseSumma').AsFloat - DataSet.FieldByName('FreeSumma').AsFloat) >
...
procedure TMlekoClosePlatRLightForm.ActionRecalLinkSumExecute(Sender: TObject);
begin
  inherited;
  Sprecalc_entity_free_sum.Close;
...
procedure TMlekoClosePlatRLightForm.ActionRecoverDolgExecute(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ActionCopyRowExecute(Sender: TObject);
begin
  inherited;
...
function TMlekoClosePlatRLightForm.DBGridFreeNaklTestBeforeMark(Sender: TObject;
  DataSet: TDataSet): Boolean;
begin
  inherited;
...
function TMlekoClosePlatRLightForm.DBGridTestBeforeMark(Sender: TObject;
  DataSet: TDataSet): Boolean;
begin
  inherited;
  result := true;
...
procedure TMlekoClosePlatRLightForm.ActionAutoCloseExecute(Sender: TObject);
var
  ln_id_to: Int64;
  i: integer;
...
procedure TMlekoClosePlatRLightForm.AutoCloseEntity(p_KeyList: TStrings; p_KeyResult: Int64; p_Entity_Type1: string; p_Entity_Type2: string; p_link_summa:
  real);
var
  i: integer;
...
procedure TMlekoClosePlatRLightForm.QuFreeNaklAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.qryLinkAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.FormCreate(Sender: TObject);
begin
  inherited;
  quDepts.Open;
  quVidRash.Open;
...
procedure TMlekoClosePlatRLightForm.SetMarkedSumma(const Value: Real);
begin
  FMarkedSumma := Value;
  StatusBar.Panels[4].Text := FloatToStr(Value);
end;

procedure TMlekoClosePlatRLightForm.SetMarkedFreeSumma(const Value: Real);
begin
  FMarkedFreeSumma := Value;
  StatusBar.Panels[1].Text := FloatToStr(Value);
end;

procedure TMlekoClosePlatRLightForm.ActionMarkMainGridExecute(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ActionLinkMarkExecute(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.DBGridFreeNaklDrawColumnCell(
...
procedure TMlekoClosePlatRLightForm.cbIsNaklRClick(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.cbIsDolgNaklRClick(Sender: TObject);
begin
  inherited;
  SQLBuilderFreeNaklGetWhere(Sender);
  SQLBuilderFreeNakl.Refresh;
...
procedure TMlekoClosePlatRLightForm.Button1Click(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ToolButton8Click(Sender: TObject);
begin
  inherited;
...
procedure TMlekoClosePlatRLightForm.ToolButton18Click(Sender: TObject);
var
...
procedure TMlekoClosePlatRLightForm.ToolButton21Click(Sender: TObject);
var
  cls_ExcelObject: string;
  WorkSheet: Variant;
  Excel: Variant;
...
procedure TMlekoClosePlatRLightForm.DBGridEh1Columns2GetCellParams(
  Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
begin
  inherited;
{
  if not (DBGridEh1.DataSource.DataSet.FieldByName('BeforeSumma').IsNull) then begin
                                                                                 if DSLinkSumma.DataSet.State <> dsEdit then DSLinkSumma.DataSet.Edit;
                                                                                DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;
                                                                              end;
}
...
procedure TMlekoClosePlatRLightForm.DBGridEh1CellClick(Column: TColumnEh);
begin
  inherited;
//  DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;
end;

procedure TMlekoClosePlatRLightForm.DSLinkSummaDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;
//  if DSLinkSumma.DataSet.State in [dsInsert, dsEdit] then DSLinkSumma.DataSet.FieldByName('BeforeSummaAccauntCurrency').AsFloat := DSLinkSumma.DataSet.FieldByName('BeforeSumma').AsFloat;
...
procedure TMlekoClosePlatRLightForm.DBGridEh1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
{

...
uses
  ...
  frXMLExl, frOLEExl,  FR_Desgn, frRtfExp,FR_DCtrl, ActnList;
...
uses UtilsDataConvert, SelectItem0, GetDate0, GetPeriodDate0, data, Sotrud0,
  GetSpravka3, GetSpravka2, EditPost0;
...
begin
 ...
 with fmOtchets do
  begin
   ...
   i := SelectItem('Печать ТТН', ['Расходная', 'Возвратная']);
   case i of
    0: frReportPrintNaklTransport.LoadFromFile('Tovarno_Transportnaya_1_0.frf');
    1: frReportPrintNaklTransport.LoadFromFile('Tovarno_Transportnaya_1_0_1.frf');
   end;
...
begin
 ...
 with fmOtchets do
  begin
   ...
   quRashod3.SQL.Clear;
   quRashod3.SQL.Add(' select Sum(Kol) Kol,Tovar.NameTovar,Company.NameCompany,Tovar.EdIzm,VidTov.VidName ');
   ...
   quRashod3.SQL.Add(' group by VidName,NameCompany,NameTovar,EdIzm,kolperpak');
...
begin
 ...
 if ParName='Kolvo' then
  ParValue:=IntToStr(Trunc(quRashodPrintKolvoKolvo.AsFloat))+' ('+IntToString(Trunc(quRashodPrintKolvoKolvo.AsFloat))+')';
 if ParName='Summa' then
  ParValue:= IntToString(Trunc(quNaklRPrintSumma.AsFloat))+' грн. '+
  format('%.2d коп.',[Round(Frac(quNaklRPrintSumma.AsFloat)*100)]);

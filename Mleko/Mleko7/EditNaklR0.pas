...
//        if quNaklR.State in [dsEdit, dsInsert] then quNaklR.Post;
      ...
      begin
        ...
        quNaklR.Open;
        quNaklR.Insert;
        DBLookupComboboxEh1.KeyValue := CurrencyAccounting;
  ...
  case Key of
    ...
    VK_INSERT:
      begin
        ...
        Key := 0;
        if isReturn then
          Ost := TfmOstatok.GetOstatok(quNaklR.FieldByName('ColnPrice').AsInteger, lvidNaklNo, lOtdelNo, isReturn, quNaklR.FieldByName('NaklNo').AsInteger)
        else
          Ost := TfmOstatok.GetOstatok(quNaklR.FieldByName('ColnPrice').AsInteger, lvidNaklNo, lOtdelNo, isReturn, quNaklR.FieldByName('NaklNo').AsInteger);
...
procedure TfmEditNaklR.DBLookupComboboxEh1Exit(Sender: TObject);
begin
  inherited;
  if quNaklR.State in [dsInsert,dsEdit] then quNaklRCurrencyHead.Value := DBLookupComboboxEh1.KeyValue;
end;

procedure TfmEditNaklR.Panel1Exit(Sender: TObject);
begin
  inherited;
  if quNaklR.State in [dsInsert,dsEdit] then quNaklR.Post;
end;

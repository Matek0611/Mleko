...
uses
  SysUtils, Windows, Messages, Classes, Graphics, Controls, StdCtrls, Forms,
  MlekoForm, DBCtrls, DB, Mask, ExtCtrls, Buttons, Dialogs, ComCtrls, DBGridEh,
  ToolWin, DBCtrlsEh, RxToolEdit, DBAccess, MsAccess, ActnList, MemDS, GridsEh,
  citDBComboEdit, cxGraphics, cxControls, cxContainer, cxEdit, cxTextEdit,
  cxMaskEdit, cxDropDownEdit, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox,
  BCFrame, BCContainer, MlkListPostArticleGroupDelayPay,
  MlkListPostArticleGroupPriceType, BCCxList, DBLookupEh;
...
type
  TfmEditPost = class(TMlekoForm)
    ...
    procedure lcRegionNameKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure qu_ContractBeforePost(DataSet: TDataSet);
    procedure DBGridEh2Columns1UpdateData(Sender: TObject; var Text: string; var
      Value: Variant; var UseText, Handled: Boolean);
    ...
    procedure RxDBGrid1Columns3EditButtons0Click(Sender: TObject; var Handled: Boolean);
    ...
    procedure DBCheckBoxEhExportInExcelFileMouseDown(Sender: TObject; Button:
    ...
    procedure DBEditEhCategoryNameCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBEditEhTimeBegCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBEditEhTimeEndCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBEditEhUnloadingCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBLookupComboboxEhRegionNoCheckDrawRequiredState(Sender: TObject;
      var DrawState: Boolean);
    procedure DBEditEhCityCheckDrawRequiredState(Sender: TObject; var DrawState: Boolean);
    procedure DBEditEhStreetCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBEditEhHouseNoCheckDrawRequiredState(Sender: TObject; var
      DrawState: Boolean);
    procedure DBEditEhXCheckDrawRequiredState(Sender: TObject; var DrawState: Boolean);
    procedure DBEditEhYCheckDrawRequiredState(Sender: TObject; var DrawState: Boolean);
    ...
    procedure DBGridEh5KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
...
uses
  data, Main, MlekoDbDsMSSQL, Post0, CategoryTT, DescriptionNaklPost,
  ListControlRequiredData, ClassifierABCDZ, SalesChannelsOfPepsico, KOATUU,
  AddressPost_CODE_Commerce_Network, AddressPostActive, GroupCutting;
...
begin
  ...
  /////////////////////////////////////////////////////////////////////////////////
  dmDataModule.OpenSQL('select EditPost,UserName from Users where UserNo = :p1_id',
    [Data.UserNo]);
  ...
  try
    ...
    quCheckControlRequiredData.SQL.Text :=
      'select isnull(RequiredData,0) as RequiredData ' +
      ' from ListUsersForRequiredData ' + '  where UserNo = :UserNo ';
    quCheckControlRequiredData.ParamByName('UserNo').Value := Data.UserNo;
    quCheckControlRequiredData.Open;

    if (quCheckControlRequiredData.RecordCount > 0) and (quCheckControlRequiredData.FieldByName
      ('RequiredData').AsBoolean = True) then
      StopControlRequiredData := True;
    ...
    begin
      ...
      while (not fmEditPost.quAddressPostForCheck.Eof) do
        ...
        begin
          ...
          Dlg.MemoListControlRequiredData.Lines.Add(fmEditPost.quAddressPostForCheckAddress.Value
            + ' :' + #10#13);

          if (fmEditPost.quAddressPostForCheckCategoryName.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Категория ТТ!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckTimeBeg.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Работа с!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckTimeEnd.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Работа по!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if ((fmEditPost.quAddressPostForCheckUnloading.Value = 0) or
             (fmEditPost.quAddressPostForCheckUnloading.IsNull))
            then
            if isInsertMode <> true then
            begin
              Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Разгрузка(мин.)!'
                + #10#13);
              i := i + 1;
              j := j + 1;
            end;
          if (fmEditPost.quAddressPostForCheckRegionNo.Value = 0) and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Район!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckCity.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Город!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckStreet.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Улица!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckHouseNo.Value = '') and (isInsertMode
            <> true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле № Дома!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckX.Value = '') and (isInsertMode <>
            true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Широта!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if (fmEditPost.quAddressPostForCheckY.Value = '') and (isInsertMode <>
            true) then
          begin
            Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Долгота!'
              + #10#13);
            i := i + 1;
            j := j + 1;
          end;
          if j = 0 then
            Dlg.MemoListControlRequiredData.Lines.Delete(Dlg.MemoListControlRequiredData.Lines.Count
              - 1);
          if j > 0 then
            Dlg.MemoListControlRequiredData.Lines.Add('' + #10#13);
...
Start:
    ...
    fmEditPost.quPostBanPay.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;

    fmEditPost.qu_Contract.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Prop.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.quTTN_Record.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Storekeeper.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Shipper.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_CarCarrier.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Confidant.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Customer.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Post_Ban.Params.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Post_Mail.ParamByName('p_PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.qu_Post_Mail.ParamByName('AddressNo').Value := fmEditPost.AddessPostForMail.KeyValue;
    fmEditPost.qu_AddressPostForMail.ParamByName('PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.quDescriptionDjobSupplier.ParamByName('PostNo').Value :=
      fmEditPost.MLKCustomDm.quPost.FieldByName('PostNo').AsInteger;
    fmEditPost.quVidExportExelFile.ParamByName('PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.quExportInExcelFile.ParamByName('PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    fmEditPost.quVidImportExelFileForNaklP.ParamByName('PostNo').Value :=
      fmEditPost.MLKCustomDm.quPost.FieldByName('PostNo').AsInteger;
    fmEditPost.qul_PostGroupCutting.ParamByName('PostNo').Value := fmEditPost.MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    ...
    fmEditPost.RxDBGrid1.OptionsEh := [dghFixed3D, dghHighlightFocus,
      dghClearSelection, dghFitRowHeightToText, dghTraceColSizing];
    fmEditPost.RxDBGrid1.Col := 1;

    if fmEditPost.ShowModal = mrOk then
    begin

      if ((CountNotRequiredData > 0) and (is_Save = True)) then
        goto Start;
...
procedure TfmEditPost.lcRegionNameKeyDown(Sender: TObject; var Key: Word; Shift:
  TShiftState);
begin
...
procedure TfmEditPost.DBGridEh2Columns1UpdateData(Sender: TObject; var Text:
  string; var Value: Variant; var UseText, Handled: Boolean);
var
  prop: Integer;
begin
  if Value then
  ...
  spSetFirmProp.Params.ParamByName('p_postNo').Value := MLKCustomDm.quPost.FieldByName
    ('PostNo').AsInteger;
...
begin
{
    ...
    if StopControlRequiredData = false then
      while (not fmEditPost.quAddressPostForCheck.Eof) do
        ...
        begin
          ...
          Dlg.MemoListControlRequiredData.Lines.Add(fmEditPost.quAddressPostAddress.Value
            + ' :' + #10#13);
          ...
          if ((fmEditPost.quAddressPostForCheckUnloading.Value = 0) or (fmEditPost.quAddressPostForCheckUnloading.IsNull))
            then
            if isInsertMode <> true then
            begin
              Dlg.MemoListControlRequiredData.Lines.Add(' Не заполнено обязательное поле Разгрузка(мин.)!'
  ...
  QuPostCheck.Connection := dmDataModule.DB;
  QuPostCheck.SQL.Clear;
  QuPostCheck.SQL.Text := 'select PostNo, Name, OKPO' + ' from Post  ' +
  ...
  QuPostCheckOKPO.SQL.Text := 'select PostNo, Name, OKPO' + ' from Post  ' +
    '  where isnull(OKPO,'''') = :OKPO ';
  QuPostCheckOKPO.ParamByName('OKPO').Value := EditOKPO.Text;
  QuPostCheckOKPO.Open;

  str1 := 'Партнер с таким ОКПО уже существует!' + #10#13 + 'Номер партнера: ' +
    QuPostCheckOKPO.FieldByName('PostNo').AsString + #10#13 + 'Имя партнера: ' +
    QuPostCheckOKPO.FieldByName('Name').AsString + #10#13 + 'ОКПО партера: ' +
    QuPostCheckOKPO.FieldByName('OKPO').AsString + #10#13 +
...
//     showmessage(str1);
  ...
  begin
    if (Application.MessageBox(PANSIChar(str1), PANSIChar('Проверка ОКПО'),
      MB_YESNO) = ID_NO) then
    begin
      QuPostCheckOKPO.Close;
      isInsertMode := false;
...

{
  ...
  if ((EditNameLong.Text = '') and (isInsertMode = true)) then
    EditNameLong.Text := EditName.Text;
  if ((Edit_Price.Text = '') and (isInsertMode = true)) then
    MLKCustomDm.quPostPriceChange.Value := 'N'; //Edit_Price.Text := 'N';
  if ((Use_Price.Text = '') and (isInsertMode = true)) then
    MLKCustomDm.quPostAccept.Value := 'Y'; //Use_Price.Text := 'Y';
  if ((cxDBLookupComboBox1.Text = '') and (isInsertMode = true)) then
    cxDBLookupComboBox1.Text := 'Активен';
  ...
  begin
    ...
    QuPostNo.SQL.Clear;
    QuPostNo.SQL.Text := 'select max(PostNo)+1 as PostNo' + ' from Post ';
    QuPostNo.Open;
    ...
    QuWork.SQL.Clear;
    QuWork.SQL.Text := 'insert into L_Post_Set_ArtGroup_Delay ' +
      '   (Set_Article_Group_Id, PostNo, Day_Delay, Day_Delay_Ext, Buh) ' +
      'values ' + '   (:Set_Article_Group_Id, :PostNo, :Day_Delay, :Day_Delay_Ext, :Buh) '
      + 'insert into L_Post_Set_ArtGroup_PayBan ' +
      '   (PostNo_PayBan, Set_Article_Group_Id_PayBan, Buh, PayBan) ' +
      'values ' + '   (:PostNo, :Set_Article_Group_Id, :Buh, isnull(:PayBan,0)) '
      + 'insert into L_Post_Set_ArtGroup_Price ' +
      '    (ColNPrice, PostNo, Set_Article_Group_Id)' + 'values ' +
      '    (:ColNPrice, :PostNo, :Set_Article_Group_Id)' + 'declare ' +
      '   @Expire_Date smalldatetime ' + '  ,@Create_Date smalldatetime ' +
      ...
      'values ' +
      '  (:PostNo,:num,:OurFirmNo,@Create_Date,@Expire_Date,:Type_id,:print_on_naklR)';
    QuWork.ParamByName('Set_Article_Group_Id').Value := 7718234909260420228;
...
//        QuWork.ParamByName('OurFirmNo').Value := 490;
 ...
 //    showmessage(QuWork.SQL.Text);
  ...
  if StopControlRequiredData = true then
    is_save := true;

  if (qu_Contract.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Contract.Post
  else
    qu_Contract.Cancel;
  if (Tsotrud_address.State in [dsInsert, dsEdit]) and (is_save = true) then
    Tsotrud_address.Post
  else
    Tsotrud_address.Cancel;
  if (quAddressPost.State in [dsInsert, dsEdit]) and (is_save = true) then
    quAddressPost.Post
  else
    quAddressPost.Cancel;
  if (quTTN_Record.State in [dsInsert, dsEdit]) and (is_save = true) then
    quTTN_Record.Post
  else
    quTTN_Record.Cancel;
  if (qu_Storekeeper.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Storekeeper.Post
  else
    qu_Storekeeper.Cancel;
  if (qu_Shipper.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Shipper.Post
  else
    qu_Shipper.Cancel;
  if (qu_CarCarrier.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_CarCarrier.Post
  else
    qu_CarCarrier.Cancel;
  if (qu_Confidant.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Confidant.Post
  else
    qu_Confidant.Cancel;
  if (qu_Customer.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Customer.Post
  else
    qu_Customer.Cancel;
  if (qu_Post_Ban.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_Post_Ban.Post
  else
    qu_Post_Ban.Cancel;
  if (qu_Post_Mail.State in [dsInsert, dsEdit]) and (cbToMail.Checked) and (is_save
    = true) then
    qu_Post_Mail.Post
  else
    qu_Post_Mail.Cancel;
  if (quPostBanPay.State in [dsInsert, dsEdit]) and (is_save = true) then
    quPostBanPay.Post
  else
    quPostBanPay.Cancel;
  if (quDescriptionDjobSupplier.State in [dsInsert, dsEdit]) and (is_save = true) then
    quDescriptionDjobSupplier.Post
  else
    quDescriptionDjobSupplier.Cancel;
  if (quExportInExcelFile.State in [dsInsert, dsEdit]) and (is_save = true) then
    quExportInExcelFile.Post
  else
    quExportInExcelFile.Cancel;
  if (quVidExportExelFile.State in [dsInsert, dsEdit]) and (is_save = true) then
    quVidExportExelFile.Post
  else
    quVidExportExelFile.Cancel;
  if (qul_PostGroupCutting.State in [dsInsert, dsEdit]) and (is_save = true) then
    qul_PostGroupCutting.Post
  else
    qul_PostGroupCutting.Cancel;
end;
...
begin
  ...
  begin
    ...
    dmDataModule.quWork.ParamByName('PostNo').AsInteger := MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    dmDataModule.quWork.Open;
    quAddressPostAddressNo.AsInteger := dmDataModule.quWork.FieldByName('AddressNo').AsInteger
      + 1;
  ...
  if quAddressPostActive.Value then
  begin
    dmDataModule.quWork.SQL.Clear;
...
begin
  ...
  begin
    if AddessPostForMail.Text = '' then
      raise Exception.Create('Не выбран адрес доставки!' + #10#13 +
        'Сделайте выбор адреса доставки!');
...
begin
  ...
  fmEditPost.quAddressPostForCheck.Close;
  fmEditPost.quAddressPostForCheck.Open;
  if fmEditPost.quAddressPostForCheck.RecordCount = 0 then
    is_save := true;
  fmEditPost.quAddressPostForCheck.First;
//  fmEditPost.quAddressPost.First;
  ...
  if StopControlRequiredData = false then
    while (not fmEditPost.quAddressPostForCheck.Eof) do
      ...
      begin
        ...
        if ((fmEditPost.quAddressPostForCheckCategoryName.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostCategoryName.Value
          = ''))) and (isInsertMode <> true) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Категория ТТ!' + #10#13;
          i := i + 1;
          ...
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Категория ТТ!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckTimeBeg.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostTimeBeg.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Работа с!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostTimeBeg.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Работа с!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckTimeEnd.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostTimeEnd.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Работа по!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostTimeEnd.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Работа по!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckUnloading.Value = 0) or (fmEditPost.quAddressPostForCheckUnloading.IsNull))
          or ((fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
          and ((fmEditPost.quAddressPostUnloading.Value = 0) or (fmEditPost.quAddressPostUnloading.IsNull)))
          then
          if isInsertMode <> true then
          begin
            ListControlRequiredDataTemp := ListControlRequiredDataTemp +
              ' Не заполнено обязательное поле Разгрузка(мин.)!' + #10#13;
            i := i + 1;
            if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            ...
            begin
              Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
                ' Не заполнено обязательное поле Разгрузка(мин.)!' + #10#13),
                Length(ListControlRequiredDataTemp));
              i := i - 1;
            end;
          end;
        if ((fmEditPost.quAddressPostForCheckRegionNo.Value = 0) or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostRegionNo.Value
          = 0)) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Район!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostRegionNo.Value <> 0) then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Район!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckCity.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostCity.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Город!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostCity.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Город!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckStreet.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostStreet.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Улица!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostStreet.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Улица!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckHouseNo.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostHouseNo.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле № Дома!' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostHouseNo.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле № Дома!' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckX.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostX.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Широта' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostX.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Широта' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;
        if ((fmEditPost.quAddressPostForCheckY.Value = '') or ((fmEditPost.quAddressPostForCheckId.Value
          = fmEditPost.quAddressPostId.Value) and (fmEditPost.quAddressPostY.Value
          = '')) and (isInsertMode <> true)) then
        begin
          ListControlRequiredDataTemp := ListControlRequiredDataTemp +
            ' Не заполнено обязательное поле Долгота' + #10#13;
          i := i + 1;
          if (fmEditPost.quAddressPostForCheckId.Value = fmEditPost.quAddressPostId.Value)
            and (fmEditPost.quAddressPostY.Value <> '') then
          begin
            Delete(ListControlRequiredDataTemp, Pos(ListControlRequiredDataTemp,
              ' Не заполнено обязательное поле Долгота' + #10#13), Length(ListControlRequiredDataTemp));
            i := i - 1;
          end;
        end;

        if i > 0 then
          ListControlRequiredData := ListControlRequiredData +
            ListControlRequiredDataTemp + #10#13;
  ...
  if ((fmEditPost.quAddressPostY.Value = '') or (fmEditPost.quAddressPostX.Value
    = '') or (fmEditPost.quAddressPostHouseNo.Value = '') or (fmEditPost.quAddressPostStreet.Value
    = '') or (fmEditPost.quAddressPostCity.Value = '') or (fmEditPost.quAddressPostRegionNo.Value
    = 0) or (fmEditPost.quAddressPostTimeEnd.Value = '') or (fmEditPost.quAddressPostTimeBeg.Value
    = '') or (fmEditPost.quAddressPostCategoryName.Value = '') or (fmEditPost.quAddressPostUnloading.Value
    = 0)) then
    j := 1
  else
    j := 0;

  if (ListControlRequiredData <> '') and (is_save = true) then
  begin
    ShowMessage(' Не заполнены обязательные поля: ' + #10#13 +
      ListControlRequiredData + #10#13 + ' Данные сохранены не будут!');
    is_Post := false;
    if j = 0 then
      ActiveControl := RxDBGrid1;
    if j <> 0 then
  ...
  if (ListControlRequiredData = '') then
    CountNotRequiredData := 0;
  if (quAddressPost.State in [dsInsert, dsEdit]) and (is_save = true) and (is_Post
    = true) then
    quAddressPost.Post
  else
    quAddressPost.Cancel;
...
begin
  if (qu_prop.State in [dsInsert, dsEdit]) and (is_save = true) then
    qu_prop.Post
  else
    qu_prop.Cancel;
end;
...
begin
  quAddressPost.ParamByName('PostNo').AsInteger := MLKCustomDm.quPost.FieldByName
    ('PostNo').AsInteger;
...
//  RxDBGrid1.Enabled := False;
  ...
  begin
    ...
    RxDBGrid1.Enabled := True;
    if IsEditRequiredData = true then
    begin
...
begin
  ...
  begin
    MessageDlg('           ' + '  Внимание! ' + #10#13 +
      ...
      'Для выхода без заполнения шаблона полностью удалите название партнера и нажмите "Отмена"',
      mtWarning, [mbOK], 0);
    Action := caNone;
    isInsertMode := false;
  end;
...
begin
  ...
  QuPostCheck.Connection := dmDataModule.DB;
  QuPostCheck.SQL.Clear;
  QuPostCheck.SQL.Text := 'select *' + '  from Post_Mail ' +
...
begin
  inherited;
  fmEditPost.qu_Post_Mail.Close;
  fmEditPost.qu_Post_Mail.ParamByName('p_PostNo').Value := MLKCustomDm.quPost.FieldByName
    ('PostNo').AsInteger;
  fmEditPost.qu_Post_Mail.ParamByName('AddressNo').Value := fmEditPost.AddessPostForMail.KeyValue;
  fmEditPost.qu_Post_Mail.Open;
...
begin
  inherited;
  quPostBanPaySET_ARTICLE_GROUP_ID_PayBan.AsLargeInt := fmEditPost.Set_Article_Group.KeyValue
    - quPostBanPayBuh.Value;
...
procedure TfmEditPost.RxDBGrid1Columns3EditButtons0Click(Sender: TObject; var
  Handled: Boolean);
begin
  inherited;
  begin
    if not (quAddressPost.State in [dsInsert, dsEdit]) then
      quAddressPost.Edit;
...
procedure TfmEditPost.quDescriptionDjobSupplierBeforePost(DataSet: TDataSet);
begin
  inherited;
...
begin
  inherited;
  if DBCheckBoxEhExportInExcelFile.Checked then
  begin
    dmDataModule.quWork.SQL.Clear;
    dmDataModule.quWork.SQL.Add('select count(*) as Cnt from L_VidExportExelFile where PostNo=:PostNo and Is_Checked = 1 and N_pp > 0');
    dmDataModule.quWork.ParamByName('PostNo').AsInteger := MLKCustomDm.quPost.FieldByName
      ('PostNo').AsInteger;
    dmDataModule.quWork.Open;
    if dmDataModule.quWork.FieldByName('Cnt').AsInteger = 0 then
    begin
      ShowMessage(' Файл отправки не настроен!' + #10#13 +
        'Настройте файл отправки и повторите потітку!');
      DBCheckBoxEhExportInExcelFile.Checked := false;
...
begin
  inherited;
  quAddressPostForCheck.ParamByName('PostNo').AsInteger := MLKCustomDm.quPost.FieldByName
    ('PostNo').AsInteger;
...
begin
  inherited;
  if DBEditEhCategoryName.Text = '' then
...
begin
  inherited;
  if DBEditEhUnloading.Text = '' then
...
begin
  inherited;
  if DBLookupComboboxEhRegionNo.Text = '' then
...
begin
  inherited;
  if DBEditEhY.Text = '' then
    DBEditEhY.Color := clRed
  else
    DBEditEhY.Color := clWindow;
end;

procedure TfmEditPost.DBEditEhCategoryNameCheckDrawRequiredState(Sender: TObject;
  var DrawState: Boolean);
begin
  inherited;
  if DBEditEhCategoryName.Text = '' then
...
procedure TfmEditPost.DBEditEhTimeBegCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhTimeBeg.Text = '' then
...
procedure TfmEditPost.DBEditEhTimeEndCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhTimeEnd.Text = '' then
...
procedure TfmEditPost.DBEditEhUnloadingCheckDrawRequiredState(Sender: TObject;
  var DrawState: Boolean);
begin
  inherited;
  if DBEditEhUnloading.Text = '' then
...
procedure TfmEditPost.DBLookupComboboxEhRegionNoCheckDrawRequiredState(Sender:
  TObject; var DrawState: Boolean);
begin
  inherited;
  if DBLookupComboboxEhRegionNo.Text = '' then
    DBLookupComboboxEhRegionNo.Color := clRed
...
procedure TfmEditPost.DBEditEhCityCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhCity.Text = '' then
...
procedure TfmEditPost.DBEditEhStreetCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhStreet.Text = '' then
...
procedure TfmEditPost.DBEditEhHouseNoCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhHouseNo.Text = '' then
...
procedure TfmEditPost.DBEditEhXCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhX.Text = '' then
...
procedure TfmEditPost.DBEditEhYCheckDrawRequiredState(Sender: TObject; var
  DrawState: Boolean);
begin
  inherited;
  if DBEditEhY.Text = '' then
...
begin
  inherited;
  if ((DBEditEhY.Text = '') or (DBEditEhX.Text = '') or (DBEditEhHouseNo.Text =
    '') or (DBEditEhStreet.Text = '') or (DBEditEhCity.Text = '') or (DBLookupComboboxEhRegionNo.Text
    = '') or (DBEditEhUnloading.Text = '') or (DBEditEhTimeEnd.Text = '') or (DBEditEhTimeBeg.Text
    = '') or (DBEditEhCategoryName.Text = '')) and (is_save = true) and (StopControlRequiredData
    = false) then
  begin
...
//                                              is_Post := false;
  ...
  if not ((DBEditEhY.Text = '') or (DBEditEhX.Text = '') or (DBEditEhHouseNo.Text
    = '') or (DBEditEhStreet.Text = '') or (DBEditEhCity.Text = '') or (DBLookupComboboxEhRegionNo.Text
    = '') or (DBEditEhUnloading.Text = '') or (DBEditEhTimeEnd.Text = '') or (DBEditEhTimeBeg.Text
    = '') or (DBEditEhCategoryName.Text = '')) and (CountNotRequiredData > 0)
    and (is_save = true) and (StopControlRequiredData = false) then
    ActiveControl := RxDBGrid1;

  if not ((DBEditEhY.Text = '') or (DBEditEhX.Text = '') or (DBEditEhHouseNo.Text
    = '') or (DBEditEhStreet.Text = '') or (DBEditEhCity.Text = '') or (DBLookupComboboxEhRegionNo.Text
    = '') or (DBEditEhUnloading.Text = '') or (DBEditEhTimeEnd.Text = '') or (DBEditEhTimeBeg.Text
    = '') or (DBEditEhCategoryName.Text = '')) and (is_save = true) then
    if (quAddressPost.State in [dsInsert, dsEdit]) then
      quAddressPost.Post;
...
procedure TfmEditPost.DBEditEhSalesChannelsOfPepsicoDblClick(Sender: TObject);
var
...
procedure TfmEditPost.cxDBLookupComboBox1PropertiesCloseUp(Sender: TObject);
begin
  inherited;
  if cxDBLookupComboBox1.Text = 'Удален' then
    dmDataModule.ExecSQL('exec sp_SetFirmProp :p1_PostNo, :p2_Id, :p3_is_prop ',
      [fmEditPost.MLKCustomDm.quPost.FieldByName('PostNo').AsInteger, 14, 1]);
...
begin
  ...
  sp_CorrectPostPayBan.ParamByName('PostNo').Value := MLKCustomDm.quPost.FieldByName
    ('PostNo').AsInteger;
...
procedure TfmEditPost.quVidImportExelFileForNaklPAfterPost(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmEditPost.DBGridEh5KeyDown(Sender: TObject; var Key: Word; Shift:
  TShiftState);
begin
  inherited;
  case Key of
    VK_DELETE:
      if Application.MessageBox('Вы уверены ?', 'Удаление', MB_YESNO or
        MB_DEFBUTTON2) = ID_YES then
      begin
        DmDataModule.OpenSQL('select * from l_PostGroupCutting where PostNo = :p1_PostNo',
          [MLKCustomDm.quPost.FieldByName('PostNo').AsInteger]);
        if DmDataModule.QFO.RecordCount > 0 then
          qul_PostGroupCutting.Delete;
      end;
  end;
end;

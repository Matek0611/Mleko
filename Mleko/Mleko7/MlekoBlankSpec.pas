...
type
  TMlekoBlankSpecForm = class(TCFLMLKListForm)
    ...
    GenInputOrderID: TIntegerField;
    GenInputOrderdocument_id: TIntegerField;
    GenInputOrderTovarNo: TIntegerField;
  ...
  private
    ...
    function GetPosWeigth(p_TovarNo:integer;var p_weight:real):boolean;
  public
    { Public declarations }
...
var
  ...
  is_only_ord: Integer;  
  ln_document_id,ln_dept_id,ln_tovarNo:integer;
 ...
 case Key of
  VK_RETURN:begin
              if GenInputOrder.FieldByName('weight_packed').AsBoolean then
               begin
                 if GetPosWeigth(GenInputOrder.FieldByName('TovarNo').AsInteger,ln_weight) then
                  begin
...
begin
  ...
  ln_tovarNo:=GenInputOrder.FieldByName('TovarNo').AsInteger;
...
//    l_boss_price:=GenInputOrder.FieldByName('PaymentPrice').AsFloat
  ...
  spCheckPrice.Params.ParamByName('tovarNo').Value:=GenInputOrder.FieldByName('tovarNo').AsInteger;
...
function TMlekoBlankSpecForm.GetPosWeigth(p_TovarNo:integer;var p_weight:real):boolean;
begin
  inherited;
...
procedure TMlekoBlankSpecForm.CheckBox1Click(Sender: TObject);
begin
  inherited;
if CheckBox1.Checked then
GenInputOrder.Filtered:= True else
GenInputOrder.Filtered:= False;
end;

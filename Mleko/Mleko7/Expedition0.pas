...
uses
  ...
  ImgList, ActnList, FR_DSet, RxMemDS, FR_DBSet, FR_Class, citCtrls,
  citmask, citDBComboEdit, GridsEh, DBGridEh, DBCtrls, SelExpedition0, cxPC,
  cxControls, cxGraphics, 
...
type
  TfmExpedition = class(TMlekoForm)
    ...
    procedure frUserDatasetVidTovExpeditionCheckEOF(Sender: TObject;
      var Eof: Boolean);
    procedure frUserDatasetVidTovExpeditionFirst(Sender: TObject);
    procedure frUserDatasetVidTovExpeditionNext(Sender: TObject);
    procedure frReportVidTovExpeditionGetValue(const ParName: string;
      var ParValue: Variant);
    procedure VidTovToCar(VidTov: integer; PrintReport: Boolean);
    procedure frReportExpeditionPrintGetValue(const ParName: string;
      var ParValue: Variant);
...
uses data, EditExpedition0, Otchets0,
  SelectItem0, SvedenieNakl0, RecycleExpedition0, BlankOst0,
  uAccountCashRashod, MlekoSelectFIrm, main, GetDate0;
...
begin
  ...
  if quVidTovExpedition.RecordCount = 0 then exit;
...
begin
  ...
  try
    ...
    begin
      if Length(TmpSql) > 0 then TmpSql := TmpSql + ' and ';
      if Length(TmpSql_) > 0 then TmpSql_ := TmpSql_ + ' and ';
...
procedure TfmExpedition.RxDBGrid1TitleBtnClick(Sender: TObject;
...
begin
  ...
  quExpedition.Close;
  case ACol of
    0: quExpedition.MacroByName('_order').Value := 'e.ExpeditionNo';
    1: quExpedition.MacroByName('_order').Value := 'ExpeditionDateGo,C.CarsName';
...
procedure TfmExpedition.frReportGetValue(const ParName: string;
  var ParValue: Variant);
begin
...
procedure TfmExpedition.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
...
procedure TfmExpedition.DBGridEh1TitleBtnClick(Sender: TObject;
...
procedure TfmExpedition.DBGridEh1DrawColumnCell(Sender: TObject;
...
begin
  ...
  else if (quExpedition.FieldByName('PayDriving').AsFloat = quExpedition.FieldByName('SumDriving').AsFloat) and
    (quExpedition.FieldByName('SumDriving').AsFloat <> 0) and (Column.Index = 7) then
  ...
  if (quExpedition.FieldByName('PayForwarding').AsFloat <> quExpedition.FieldByName('SumForwarding').AsFloat) and (Column.Index = 8) then
    DbGridEh1.Canvas.Brush.Color := clRed
  else if (quExpedition.FieldByName('PayForwarding').AsFloat = quExpedition.FieldByName('SumForwarding').AsFloat) and
    (quExpedition.FieldByName('SumForwarding').AsFloat <> 0) and (Column.Index = 8) then
    DbGridEh1.Canvas.Brush.Color := clMoneyGreen;
...
procedure TfmExpedition.frUserDatasetVidTovExpeditionCheckEOF(
  Sender: TObject; var Eof: Boolean);
begin
  EOF := (mdExpedition.FieldCount = Coln);
end;

procedure TfmExpedition.frUserDatasetVidTovExpeditionFirst(
  Sender: TObject);
begin
  Coln := 5;
end;
...
procedure TfmExpedition.frReportVidTovExpeditionGetValue(
...
procedure TfmExpedition.frReportExpeditionPrintGetValue(
  const ParName: string; var ParValue: Variant);
begin
  if ParName = 'Date' then
    ParValue := quExpeditionExpeditionDateGo.AsDateTime;
...
begin
  ...
  if DateBlock >= quExpeditionExpeditionDateGo.AsDateTime
   then begin
          msg:= 'Удаление запрещено!' + #10#13 +
                'Период заблокирован';
          raise Exception.Create(msg);
        end;

  if not quExpeditionroute_id.IsNull then begin
                                            msg:= 'Удаление запрещено!' + #10#13 +
                                                  'Для возможности удаления необходимо отвязать маршрут';
                                            raise Exception.Create(msg);
                                          end;
...
begin
  ...
  quNaklR.Open;
  i := SelectItem('Печать', ['Загрузка', 'Список магазинов', 'Тарная накладная', 'Загрузка по виду', 'Список накладных', 'Отчет по ходкам']);
  case i of
    0:
      begin
        S := '';
        ...
        begin
          ...
          S := '(' + S + ')';
          LoadCarNew(1, S, S1, False, True, 'Код:' + quExpeditionExpeditionNo.AsString + ' ' + quExpeditionCarsName.AsString,
            quExpeditionShipping_AgentName.AsString, quExpeditionExpeditionDateGo.AsDateTime);
    ...
    0:
      begin
        ...
        begin
          S := Copy(S, 1, Length(S) - 1);
          S := '(' + S + ')';
          LoadCarNew(1, S, S1, False, False, quExpeditionCarsName.AsString, quExpeditionShipping_AgentName.AsString,
            quExpeditionExpeditionDateGo.AsDateTime);
        end;
        if S2 <> '' then
        begin
    ...
    3:
      begin
        ...
        begin
          S := Copy(S, 1, Length(S) - 1);
          S := '(' + S + ')';
          LoadCarNoTTNew(1, S, S1, False, False, quExpeditionCarsName.AsString, quExpeditionShipping_AgentName.AsString,
            quExpeditionExpeditionDateGo.AsDateTime);
        end;
        quWork.Close;
        quWork.SQL.Clear;
...
Expedition_Number:= MSQuery_Expedition_response_id.FieldByName('activity_type_id').Value;
   ...
   end else if ((StrToInt(Expedition_Number) >= 3) AND (StrToInt(Expedition_Number) <= 10) or (StrToInt(Expedition_Number) = 23) or (StrToInt(Expedition_Number) = 25)) then
   begin
       Number_response_dept_id:=IntToStr(4);
   end else if ((StrToInt(Expedition_Number) >= 11) AND (StrToInt(Expedition_Number) <= 13) ) then
   begin
       Number_response_dept_id:=IntToStr(11);
   end else if ((StrToInt(Expedition_Number) >= 14) AND (StrToInt(Expedition_Number) <= 16) ) then
   begin
       Number_response_dept_id:=IntToStr(14);

    end else if  ((StrToInt(Expedition_Number) >= 17) AND (StrToInt(Expedition_Number) <= 20) ) then
   begin
       Number_response_dept_id:=IntToStr(17);

   end else if ((StrToInt(Expedition_Number) >= 27) AND (StrToInt(Expedition_Number) <= 29) or (StrToInt(Expedition_Number) = 31)) then
   begin
       Number_response_dept_id:=IntToStr(20);
   end else if ((StrToInt(Expedition_Number) = 30)) then
   begin
...
begin
  ...
  if quFreeNakl.RecordCount>0 then tsFreeNakl.Color:=clFuchsia else tsFreeNakl.Color:=clBtnFace;
end;
...
procedure TfmExpedition.DBGridEh1Columns10UpdateData(Sender: TObject;
  var Text: string; var Value: Variant; var UseText, Handled: Boolean);
var
...
begin
  ...
  begin
    ...
    begin
      if
        (UpperCase(DBGridEh1.Columns[i].FieldName) = 'PAYDRIVING')
        or
        (UpperCase(DBGridEh1.Columns[i].FieldName) = 'PAYFORWARDING')
        or
        (UpperCase(DBGridEh1.Columns[i].FieldName) = 'DISTANCE') then DBGridEh1.Columns[i].ReadOnly := false;
    end;
  end;
end;
...
begin
  ...
  CheckDeyExpedition.SQL.Text:='select isnull(count(route_id),0) as is_Route ' +
                               ' from Expedition                             ' +
                               '  where ExpeditionDateGo = :ExpeditionDateGo ';
  ...
  if CheckDeyExpedition.FieldByName('is_Route').Value > 0
   then begin
          msg:= 'Удаление запрещено!' + #10#13 +
                'Для возможности удаления необходимо отвязать маршруты от всех ходок';
          raise Exception.Create(msg);
        end;
  ...
  ListDeletedExpedition.SQL.Text := 'select * from Expedition                     ' +
                                    '  where ExpeditionDateGo = :ExpeditionDateGo ';
  ...
  while not ListDeletedExpedition.Eof do
    begin
      dmDataModule.quWork.SQL.Clear;
      dmDataModule.quWork.SQL.Add('if (select VidNaklNo from NaklR where ExpeditionNo = :ExpeditionNo1) = 3 ' +
                                  ' update NaklR                                                            ' +
                                  '  set ExpeditionNo = 0                                                   ' +
                                  '  set InCar = 0                                                          ' +
                                  ' where ExpeditionNo = :ExpeditionNo  ');
      ...
      dmDataModule.quWork.SQL.Clear;
      dmDataModule.quWork.SQL.Add('delete from L_ExpeditionWithLogistic ' +
                                  ' where ExpeditioinNo = :ExpeditionNo ');
      dmDataModule.quWork.ParamByName('ExpeditionNo').Value := ListDeletedExpedition.FieldByName('ExpeditionNo').Value;
      dmDataModule.quWork.Prepare;
      dmDataModule.quWork.Execute;

      dmDataModule.quWork.SQL.Clear;
      dmDataModule.quWork.SQL.Add('delete from Expedition                  ' +
                                  ' where ExpeditionNo = :OLD_ExpeditionNo ');
      dmDataModule.quWork.ParamByName('ExpeditionNo').Value := ListDeletedExpedition.FieldByName('ExpeditionNo').Value;
      dmDataModule.quWork.Prepare;
      dmDataModule.quWork.Execute;

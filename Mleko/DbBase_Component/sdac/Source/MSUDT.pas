...
const
  ...
  CoreLab_Sdac_UDTProxyMajorVersion = 1;
  CoreLab_Sdac_UDTProxyMinorVersion = 0;

  LIBID_CoreLab_Sdac_UDTProxy: TGUID = '{A7C92A4E-7EFD-4A24-9610-83BA8A7B09EC}';
...
begin
  ...
  while p <> nil do begin
    ...
    if ExcepInfo.bstrHelpFile <> '' then
      ExcepInfo.bstrHelpFile := #$D#$A + ExcepInfo.bstrHelpFile;
    ExcepInfo.bstrHelpFile := p.HelpLink + ExcepInfo.bstrHelpFile;
    p := p.InnerException;
  end;
  ...
  while p <> nil do begin
    if ExcepInfo.bstrSource <> '' then
      ExcepInfo.bstrSource := #$D#$A + ExcepInfo.bstrSource;
    ExcepInfo.bstrSource := '[' + WideString(p.Source) + ']' + WideString(p.StackTrace) + ExcepInfo.bstrSource;
    p := p.InnerException;
  end;
  ...
  while p <> nil do begin
    if ExcepInfo.bstrDescription <> '' then
      ExcepInfo.bstrDescription := #$D#$A + ExcepInfo.bstrDescription;
    ExcepInfo.bstrDescription := p.Message + ExcepInfo.bstrDescription;
    p := p.InnerException;
  end;
...
procedure TUDTDispatcher.InitUDTProxy;
  ...
  begin
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}', '', 'CoreLab.Sdac.UDTProxy', True, HKEY_CLASSES_ROOT);
    ...
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}\InprocServer32', 'Class', 'CoreLab.Sdac.UDTProxy', True, HKEY_CLASSES_ROOT);
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}\InprocServer32', 'Assembly', 'CoreLab.Sdac.UDTProxy, Version=1.0.0.0, Culture=neutral, PublicKeyToken=09af7300eec23701', True, HKEY_CLASSES_ROOT);
    ...
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}\InprocServer32\1.0.0.0', 'Class', 'CoreLab.Sdac.UDTProxy', True, HKEY_CLASSES_ROOT);
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}\InprocServer32\1.0.0.0', 'Assembly', 'CoreLab.Sdac.UDTProxy, Version=1.0.0.0, Culture=neutral, PublicKeyToken=09af7300eec23701', True, HKEY_CLASSES_ROOT);
    ...
    AddRegistryKey('CLSID\{67B585B0-589C-3168-AD3F-FCA62F2CD42E}\ProgId', '', 'CoreLab.Sdac.UDTProxy', True, HKEY_CLASSES_ROOT);
...
begin
  if not CompareGuid(IID, GUID_NULL) then
    ...
    for i := 0 to ParamCount - 1 do
      VarParams[i] := Variant(TVariantArgList(TDispParams(Params).rgvarg^)[ParamCount - 1 - i]);

...
type
  ...
  public
    ...
    procedure CreateServerObjects(Contract: string = 'DEFAULT');
...
begin
  ...
  {$ENDIF}
    ...
    {$ELSE}
      Reader := XMLTextReader.Create(Encoding.Unicode.GetWideString(FBody));
    {$ENDIF}
...
procedure TMSServiceBroker.CreateServerObjects(Contract: string = 'DEFAULT');
begin
  Connection.ExecSQL(
    'IF NOT EXISTS (SELECT * FROM sys.service_queues WHERE name = ''' + Service + '_QUEUE'') CREATE QUEUE ' + BracketIfNeed(Service + '_QUEUE') + #$D#$A +
    'IF NOT EXISTS (SELECT * FROM sys.services WHERE name = ''' + Service + ''') CREATE SERVICE ' + BracketIfNeed(Service) + ' ON QUEUE ' + BracketIfNeed(Service + '_QUEUE') + '([' + Contract + '])', []);
end;

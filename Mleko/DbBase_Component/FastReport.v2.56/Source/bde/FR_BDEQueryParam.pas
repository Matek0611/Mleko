...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, DBTables, FR_BDEQuery;

type
  TfrBDEParamsForm = class(TForm)
    GroupBox1:TGroupBox;
    Label2:TLabel;
    ParamsLB:TListBox;
    TypeCB:TComboBox;
...
function TfrBDEParamsForm.CurParam:Integer;
var
  i:Integer;
  s:String;
begin
...
procedure TfrBDEParamsForm.FormShow(Sender:TObject);
var
  i:Integer;
begin
...
procedure TfrBDEParamsForm.FormHide(Sender:TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrBDEParamsForm.ParamsLBClick(Sender:TObject);
var
  i:Integer;
begin
  TypeCB.ItemIndex:=-1;
...
procedure TfrBDEParamsForm.ValueEExit(Sender:TObject);
begin
  QueryComp.ParamText[CurParam]:= ValueE.Text;
end;

procedure TfrBDEParamsForm.TypeCBChange(Sender:TObject);
begin
...
procedure TfrBDEParamsForm.ValueRBClick(Sender:TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled:= True;
...
procedure TfrBDEParamsForm.AssignRBClick(Sender:TObject);
begin
  if FBusy then Exit;
  ValueE.Text:= '';
  ValueE.Enabled:= False;
...
procedure TfrBDEParamsForm.Localize;
begin
...
procedure TfrBDEParamsForm.FormCreate(Sender:TObject);
begin
  Localize;
end;

procedure TfrBDEParamsForm.VarSBClick(Sender:TObject);
begin

...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, DBXpress, SqlExpr, FR_DBXQuery;

type
  TfrDBXParamsForm = class(TForm)
    GroupBox1:TGroupBox;
    Label2:TLabel;
    ParamsLB:TListBox;
    TypeCB:TComboBox;
...
function TfrDBXParamsForm.CurParam:Integer;
var
  i:Integer;
  s:String;
begin
...
procedure TfrDBXParamsForm.FormShow(Sender:TObject);
var
  i:Integer;
begin
...
procedure TfrDBXParamsForm.FormHide(Sender:TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure TfrDBXParamsForm.ParamsLBClick(Sender:TObject);
var
  i:Integer;
begin
  TypeCB.ItemIndex:=-1;
...
procedure TfrDBXParamsForm.ValueEExit(Sender:TObject);
begin
  QueryComp.ParamText[CurParam]:= ValueE.Text;
end;

procedure TfrDBXParamsForm.TypeCBChange(Sender:TObject);
begin
...
procedure TfrDBXParamsForm.ValueRBClick(Sender:TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled:= True;
...
procedure TfrDBXParamsForm.AssignRBClick(Sender:TObject);
begin
  if FBusy then Exit;
  ValueE.Text:= '';
  ValueE.Enabled:= False;
...
procedure TfrDBXParamsForm.Localize;
begin
...
procedure TfrDBXParamsForm.FormCreate(Sender:TObject);
begin
  Localize;
end;

procedure TfrDBXParamsForm.VarSBClick(Sender:TObject);
begin

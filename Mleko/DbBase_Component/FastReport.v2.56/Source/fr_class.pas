...
// helper methods
  ...
  public
    ...
    function DoClick(Index:Integer; pt:TPoint; Click:Boolean; var Cursor:TCursor; var mess:string):Boolean;
...
{$ENDIF}
  ...
  published
    ...
    property UseDefaultDataSetName:Boolean read FUseDefaultDataSetName write FUseDefaultDataSetName default False;
...
function frRegisterExportFilter(Filter:TfrExportFilter;
  const FilterDesc, FilterExt:String):boolean;
...
begin
  for i:= 0 to frFiltersCount-1 do
    if (frFilters[i].Filter.ClassName = Filter.ClassName) and
       (frFilters[i].FilterExt = FilterExt) then
       begin
         Result:= false;
         Exit;
       end;
...
var
  i, j:Integer;
begin
  for i:= frFiltersCount-1 downto 0 do
    if frFilters[i].Filter.ClassName = Filter.ClassName then
    begin
      if i < frFiltersCount-1 then
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        yy:= y;
      if (CurBand<>nil) and (DocMode<>dmDesigning) then
        bandstretched:=((CurBand.Flags and flStretched)<>0);

      if ((theLObj.y = yy) or ((theLObj.y = y)and bandstretched)) and
    ...
    if s.Count > 0 then
      Result:= TfrView(s.Objects[s.Count-1]) else
      Result:= nil;
    s.free;
  end;
  ...
  begin
    ...
    begin
      ...
      if (CurBand<>nil) and (DocMode<>dmDesigning) then
        bandstretched:=((CurBand.Flags and flStretched)<>0);
      if ((theObj.y = yy) or ((theObj.y = y)and bandstretched)) and
    ...
    if s.Count > 0 then
      Result:= TfrView(s.Objects[0]) else
      Result:= nil;
    s.free;
  end;
...
begin
  ...
  if s = 'TfrxDBLookupControl' then
    s:= 'TfrxDBLookupComboBox';
...
procedure TfrNonVisualControl.SaveToFR3Stream(Stream:TStream);
...
function TfrStretcheable.DoMethod(MethodName:String; Par1, Par2, Par3:Variant):Variant;
begin
  Result:= inherited DoMethod(MethodName, Par1, Par2, Par3);
...
begin
  ...
  begin
    s:= Memo[i];
    if (s<>'') and ((Flags and flTextOnly) = 0) then
      ExpandVariables(s);
    if s<>'' then
        ...
        begin
          ...
          begin
            ...
            for i:= 1 to Length(str) do
              if (str[i] = ' ') or (str[i] = #$A0) then Inc(n);
...
// fix is here
  DrawText(Canvas.Handle, PChar(s), Length(s), CalcRect, DT_CALCRECT+DT_NOPREFIX);
//
...
begin
  ...
  begin
    ...
    CanExpandVar:= False;
  if ((DrawMode<>drPart) and CanExpandVar) or FExpand then
      ExpandMemoVariables;
...
begin
  ...
  begin
    ...
    if ((Flags and flSuppressRepeated) = 0) or (Memo1.Text<>LastValue) then
      Result:= VHeight;
    if MasterReport.DoublePass and not MasterReport.FinalPass then
      if (Flags and flSuppressRepeated)<>0 then
        LastValue:= Memo1.Text;
  end;
  RestoreCoord;
...
begin
  TextHeight:=-Font.Height+LineSpacing;
  Result:= TextHeight+gapy * 2+Round(FrameWidth * 2);
end;

function TfrMemoView.LostSpace:Integer;
begin
  TextHeight:=-Font.Height+LineSpacing;
  Result:= (dy-Round(FrameWidth * 2)-gapy * 2) mod TextHeight+gapy * 2+Round(FrameWidth * 2);
end;
...
begin
  ...
  if (Flags and flBandBreaked)<>0 then
...
begin
  ...
  if Index = 'CENTER' then
    ...
    BlobType:= Value
  else if Index = 'PICTURE' then
    Picture.Assign(TPicture(Longint(Value)))
end;
...
{$WARNINGS OFF}
  ...
  begin
    ...
    if (aGraph is TBitmap)
{$IFDEF JPEG}
    or (aGraph is TJPEGImage)
{$ENDIF} then
    begin
     try
...
{$ENDIF}
      ...
      PrintBitmap(Canvas, DestRect, Bitmap);
     finally
       Bitmap.Free;
     end;
    end;
...
{$ENDIF}
{$IFDEF RX}
    else if Picture.Graphic is TGIFImage then
      b:= pkGIF
{$ENDIF};
  Stream.Write(b, 1);
...
{$ENDIF}
    2:Graphic:= TIcon.Create;
    3:Graphic:= TMetafile.Create;
  end;
  Picture.Graphic:= Graphic;
  if Graphic<>nil then
    Graphic.Free;
...
function TfrLineView.LostSpace:Integer;
begin
  Result:= 0;
end;
...
begin
  ...
  if Result > CalculatedHeight then
  ...
  try
    if t.Parent = Self then
      ...
      begin
        ...
        oy:= t.y; Inc(t.y, y);
// if MasterReport.EMFPages.Count>0 then
...
begin
  ...
  if (MaxY-Parent.CurY) > CalculatedHeight then
    ddy:= MaxY-Parent.CurY
  else
    ddy:= CalculatedHeight;
  Inc(Parent.CurY, ddy);
  Parent.LastBand:= nil;
end;
...
begin
  ...
  begin
    ...
    if Pos(#1, t.Memo1.Text) = 0 then
      t.FExpand:= True; //Fixed
    with t do
      if BandAlign = baBottom then
        y:= Self.CalculatedHeight-dy;
  end;
        ...
        if (t.y >= 0) and (t.y < dy) then
          ...
          begin
            ...
            begin
              olddy:= t.dy;
              t.dy:= dy-t.y+1;
              ls:= TfrStretcheable(t).LostSpace;
...
// new page was requested in script
  ...
  begin
    ...
    begin
      if dy > CalculatedHeight then
        CalculatedHeight:= dy;
      if not (Typ in [btPageFooter, btColumnFooter, btOverlay, btNone]) then
      begin
        CHeight:= DoCalcHeight;
        if (Parent.CurY+CHeight > Parent.CurBottomY) and not PageBreak then
        begin
         if not ((CHeight > Parent.CurBottomY-Parent.TopMargin) and (Parent.CurY = Parent.TopMargin)) then
          begin
          Result:= True;
          if Parent.Skip then
            Exit else
...
begin
  ...
  ObjectsToPage(Index);
  for i:= 0 to p.Page.Objects.Count-1 do
    TfrView(p.Page.Objects[i]).ExportData;

  p.Page.Free;
  p.Page:= nil;
end;

procedure TfrReport.BuildPage(Index:Integer);
begin
  EMFPages.ObjectsToPage(Index);
end;
...
begin
  ...
  begin
    if Stream is TMemoryStream then
      TMemoryStream(Stream).Clear else
      Stream.Position:= 0;
    frVersion:= frCurrentVersion;
...
begin
  ...
  repeat
    ...
    Inc(i);
  until i >= c;
  AStream.Read(Parent.ReportGeneratorVersion, 1);
end;
...
begin
  ...
  AStream.Write(Parent.ReportGeneratorVersion, 1);
end;

function TfrEMFPages.DoClick(Index:Integer; pt:TPoint; Click:Boolean;
  var Cursor:TCursor; var mess:string):Boolean;
var
  p:PfrPageInfo;
  t:TfrView;
  i:Integer;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              mess:= '';
              ShellExecute(GetDesktopWindow, nil, PChar(t.Tag), nil, nil, sw_ShowNormal)
            end;
            Result:= True;
          end;
        end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Assigned(CurReport.FOnCustomizeObject) then
          if t is TfrNonVisualControl then
            CurReport.FOnCustomizeObject(TfrNonVisualControl(t).Component);

        if not (t is TfrNonVisualControl) then
        begin
          TfrControl(t).PlaceControl(Form);
          t.Prop['Visible']:= t.Visible;
          HasControls:= True;
        end;
      end;
...
//
  ...
  else if MasterReport.DoublePass and MasterReport.FinalPass and
    (PageType = ptReport) then
  begin
    for i:= 0 to Objects.Count-1 do
    begin
      t:= Objects[i];
    ...
    for b:= btReportTitle to btNone do
      if Assigned(Bands[b]) and Assigned(Bands[b].Dataset) then
        Bands[b].Dataset.First;
  end;
end;
...
begin
  ...
  begin
    ...
    if FDFMStream = nil then
      ...
      if FDFMStream is TMemoryStream then
        TMemoryStream(FDFMStream).Clear;
...
begin
  ...
  if FStoreInDFM then
  begin
    if not Assigned(FDFMStream) then Exit;
  ...
  begin
    ...
    begin
      if s = 'VALUE' then
        ...
        v:= SavedAllPages
      else if s = frRepInfo[0] then
        v:= ReportComment
      else if s = frRepInfo[1] then
        v:= ReportName
      else if s = frRepInfo[2] then
        v:= ReportAutor
      else if s = frRepInfo[3] then
        v:= ReportVersionMajor
      else if s = frRepInfo[4] then
        v:= ReportVersionMinor
      else if s = frRepInfo[5] then
        v:= ReportVersionRelease
      else if s = frRepInfo[6] then
        v:= ReportVersionBuild
      else if s = frRepInfo[7] then
        v:= ReportCreateDate
      else if s = frRepInfo[8] then
        v:= ReportLastChange
...
begin
  ...
  ReportGeneratorVersion:= frtName+11;
end;
...
{$ENDIF}
    ...
    begin
      ...
      if l>0 then
      begin
        SetLength(ReportAutor, l);
        Stream.Read(ReportAutor[1], l);
      end;
      Stream.Read(l, 2);
      if l>0 then
      begin
    ...
    begin
        ...
        if not ((PassForm.ShowModal = mrOk) and (PassForm.E1.Text = ReportPassword)) then
        begin
          ClearAttribs;
          FileName:= frLoadStr(SUntitled);
          Pages.Clear;
...
{$ENDIF}
  ...
  b:= $FC;
  Stream.Write(b, 1);
  l:= Length(ReportComment);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportComment[1], l);
  l:= Length(ReportName);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportName[1], l);
  l:= Length(ReportAutor);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportAutor[1], l);
  l:= Length(ReportVersionMajor);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportVersionMajor[1], l);
  l:= Length(ReportVersionMinor);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportVersionMinor[1], l);
  l:= Length(ReportVersionRelease);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportVersionRelease[1], l);
  l:= Length(ReportVersionBuild);
  Stream.Write(l, 2);
  if l>0 then
    Stream.Write(ReportVersionBuild[1], l);
  l:= Length(ReportPassword);
  m:= l * 2;
  Stream.Write(m, 2);
  if l>0 then
  begin
    Buf:= StrToHex(ReportPassword);
    Stream.Write(Buf[1], m);
  end;
  ReportGeneratorVersion:= frtName+11;
  Stream.Write(ReportGeneratorVersion, 1);
  Stream.Write(ReportPasswordProtected, SizeOf(Boolean));
  Stream.Write(ReportCreateDate, SizeOf(TDateTime));
  ReportLastChange:= Now;
  Stream.Write(ReportLastChange, SizeOf(TDateTime));
end;
  ...
  begin
    ...
    if not Assigned(FOnProgress) and FShowProgress then
      ...
      begin
        ...
        Result:= Show_Modal(Self)<>mrCancel;
      end
    else
...
begin
  ...
  if Collate then
    Printer.Copies:= 1
    else if pcCopies in Printer.Capabilities then Printer.Copies:= Copies;
  Printer.BeginDoc;
  f:= True;
  if Collate then
  begin
    ...
    for i:= 0 to EMFPages.Count-1 do
      if (pgList.Count = 0) or (pgList.IndexOf(IntToStr(i+1))<>-1) then
        if pcCopies in Printer.Capabilities then begin
          PrintPage(i);
  ...
  try
    ...
    if frDesigner.Modified then
      if Application.MessageBox(PChar(frLoadStr(SSaveChanges)+'?'),
        PChar(frLoadStr(SConfirm)), mb_YesNo+mb_IconQuestion) = mrYes then
...
procedure TfrCompositeReport.Clear;
begin
  Reports.Clear;
end;
...
// frProgressForm.FormStyle:= fsStayOnTop;
  ...
  frDialogForm.Position:= poDesigned;
...
begin
  ...
  frUnRegisterFunctionLibrary(TDatabaseFunctionLibrary);
  frParser.Free;
...
var
  rsAggregate, rsDateTime, rsString, rsOther, rsInfo, rsMath, rsBool, rsInterpr:String;
begin
  inherited Create;
  with List do
  begin
...
// Report info functions
  AddFunctionDesc('REPORTCOMMENT', rsInfo, frLoadStr(frRes+2623));
  AddFunctionDesc('REPORTNAME', rsInfo, frLoadStr(frRes+2624));
  AddFunctionDesc('REPORTAUTOR', rsInfo, frLoadStr(frRes+2625));
  AddFunctionDesc('VMAJOR', rsInfo, frLoadStr(frRes+2626));
  AddFunctionDesc('VMINOR', rsInfo, frLoadStr(frRes+2627));
  AddFunctionDesc('VRELEASE', rsInfo, frLoadStr(frRes+2628));
  AddFunctionDesc('VBUILD', rsInfo, frLoadStr(frRes+2629));
  AddFunctionDesc('REPORTDATE', rsInfo, frLoadStr(frRes+2630));
  AddFunctionDesc('REPORTLASTCHANGE', rsInfo, frLoadStr(frRes+2631));
...
constructor TDatabaseFunctionLibrary.Create;
begin
  inherited Create;
  with List do
  begin
  ...
  AddFunctionDesc('FIELDISNULL', frLoadStr(frRes+2620), frLoadStr(frRes+2621));
end;
...
begin
  val:= 0;
  case FNo of
     0:val:= frFieldIsNull(frParser.Calc(p1));
  end;
end;

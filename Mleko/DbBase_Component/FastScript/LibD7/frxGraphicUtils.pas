...
// call these methods before and after doing something
    ...
    property UseDefaulCharset: Boolean read FUseDefaulCharset write FUseDefaulCharset;
  end;
...
uses frxPrinter, SyncObjs {added by Samuray};
...
var
  ...
  var
    ...
    Size: TSize;
  begin
    if {(Win32Platform <> VER_PLATFORM_WIN32_NT) or ((Canvas.Font.Charset <> DEFAULT_CHARSET) and not FUseDefaulCharset)} DefCharset then
    begin
...
begin
  ...
  if {((Canvas.Font.Charset <> DEFAULT_CHARSET) and not FUseDefaulCharset) or (Win32Platform <> VER_PLATFORM_WIN32_NT)} DefCharset then
  begin
...
// get the intercharacter spacing table and calculate the width
  ...
  try
    sz.cx := FHTMLTags.FillCharSpacingArray(FTempArray, s, FCanvas,
      FHTMLTags.Count - 1, CharSpacing, True, FUseDefaulCharset);
  finally
    FCanvas.Unlock;
  end;
...
begin
  ...
  addI := 0;
  AddNext := True;
  if ((C.Font.Charset <> DEFAULT_CHARSET) and not FUseDefaulCharset) or (Win32Platform <> VER_PLATFORM_WIN32_NT) then
  begin
  ...
  try
      ...
      PrnSz := FHTMLTags.FillCharSpacingArray(FTempArray, s, FCanvas, LineIndex,
        Round(FCharSpacing * FPPI), False, FUseDefaulCharset) - Round(FCharSpacing * FPPI);
      Sz := FHTMLTags.FillCharSpacingArray(FTempArray, s, C, LineIndex,
        Round(FCharSpacing * FScaleX), False, FUseDefaulCharset) - Round(FCharSpacing * FScaleX);                      //!Den
      C.Font.Style := Style;
    ...
    repeat
      ...
      if ((C.Font.Charset = DEFAULT_CHARSET) or FUseDefaulCharset) and (Win32Platform = VER_PLATFORM_WIN32_NT) then
        if FWysiwyg then
...
begin
  ...
  ratio := FDefPPI / FScrPPI;
  Result := tm.tmInternalLeading/ratio;
  Result :=  Result + (PrnSz / ratio + FLineSpacing) * n - FLineSpacing;
end;

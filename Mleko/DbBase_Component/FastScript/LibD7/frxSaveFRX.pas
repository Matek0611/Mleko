...
uses
  ...
  ComCtrls,
  frxClass, frxXML, frxUnicodeUtils, frxVariables, frxUtils, frxEngine,
{$IFDEF Delphi6}
  Variants,
{$ENDIF}
  frxBarcode, frxChBox, frxRich, frxNetUtils, frxDesgn, frxDCtrl;

type
  TfrxSaveFRX = class(TfrxCustomSavePlugin)
  private
    Report: TfrxReport;
  ...
  public
    constructor Create;
    procedure Save(Report: TfrxReport; const FileName: String); override;
  end;
...
constructor TfrxSaveFRX.Create;
begin
  FileFilter := 'FastReport.Net file (*.frx)|*.frx';
end;
...
begin
  ...
  root.Prop['Text'] := UTF8Encode(memo.Text);
...
procedure TfrxSaveFRX.Save(Report: TfrxReport; const FileName: String);
var
  doc: TfrxXMLDocument;
begin
  Self.Report := Report;
...
initialization
  frxSavePlugin := TfrxSaveFRX.Create;

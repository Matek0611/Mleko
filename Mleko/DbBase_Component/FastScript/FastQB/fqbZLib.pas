...
type
  ...
  TZStreamRec = packed record
    next_in: PByte;//AnsiChar; // next input byte
    avail_in: Longint; // number of bytes available at next_in
    total_in: Longint; // total nb of input bytes read so far
    next_out: PByte;//AnsiChar; // next output byte should be put here
    avail_out: Longint; // remaining free space at next_out
    total_out: Longint; // total nb of bytes output so far
    msg: PByte;//AnsiChar; // last error message, NULL if no error
  ...
  private
    ...
    FBuffer: array[Word] of Byte;//AnsiChar;
  protected
  ...
  try
    ...
    try
      ...
      begin
        ...
        zstream.next_out := PByte{AnsiChar}(Integer(outBuffer) + zstream.total_out);

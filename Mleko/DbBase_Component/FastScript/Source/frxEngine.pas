    ...
    begin
      ...
      begin
        ...
        begin
          if FirstTime then
          begin
            if b.KeepHeader and (b.FHeader <> nil) and not FIsFirstBand then
            begin
              FKeepHeader := True;
              StartKeep(b);
            end;
...
begin
  ...
  begin
   { use own CurX - we may be inside subreports now }
    ...
    if Band <> nil then
      if not FKeepHeader and not FHeaderList[i].IsInKeepList or FKeepFooter
        ...
        if FOutputTo = nil then
          ...
          begin
            if not FKeeping then
              CheckSuppress(Band);
            IsKeept := FKeeping;
            if not((Band is TfrxChild) and FCallFromAddPage) then {endless loop fix}
              NewColumn;
            if (Band.Height > FreeSpace) and IsKeept then {need for group with keeptogether when objects moved from previous page}
            begin
              FCurBand := Band;
              NewColumn;
            end;

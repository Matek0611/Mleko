...
{ Report Objects }
 ...
 TfrxView = class(TfrxReportComponent)
  ...
  public
    ...
    property UseDefaulCharset: Boolean read FUseDefaulCharset write FUseDefaulCharset default False;
...
begin
  Result := nil;
  if (Report <> nil) and not Report.EngineOptions.UseGlobalDataSetList then
  begin
    DSItem := Report.EnabledDataSets.Find(DSName);
    if DSItem <> nil then Result := DSItem.FDataSet;
  end
  else
    Result := frxFindDataSet(DataSet, DSName, Report);
end;
...
begin
  FRunAlways := not Report.EngineOptions.DestroyForms;
  FControl := AControl;
...
begin
  if Report <> nil then
    Report.DoNotifyEvent(Self, FOnClick, FRunAlways);
end;

procedure TfrxDialogControl.DoOnDblClick(Sender: TObject);
begin
  if Report <> nil then
    Report.DoNotifyEvent(Self, FOnDblClick, FRunAlways);
end;

procedure TfrxDialogControl.DoOnEnter(Sender: TObject);
begin
  if Report <> nil then
    Report.DoNotifyEvent(Self, FOnEnter, FRunAlways);
end;

procedure TfrxDialogControl.DoOnExit(Sender: TObject);
begin
  if Report <> nil then
    Report.DoNotifyEvent(Self, FOnExit, FRunAlways);
end;
...
begin
  ...
  begin
    Report.DoParamEvent(FOnKeyDown, v, FRunAlways);
    Key := v[1];
  end;
end;
...
begin
  ...
  begin
    Report.DoParamEvent(FOnKeyPress, v, FRunAlways);
...
begin
  ...
  begin
    Report.DoParamEvent(FOnKeyUp, v, FRunAlways);
    Key := v[1];
  end;
end;
...
begin
  ...
  if Report <> nil then
    Report.DoParamEvent(FOnMouseDown, v, FRunAlways);
end;
...
begin
  ...
  if Report <> nil then
    Report.DoParamEvent(FOnMouseMove, v, FRunAlways);
end;
...
begin
  v := VarArrayOf([Integer(Self), Button, ShiftToByte(Shift), X, Y]);
  if Report <> nil then
    Report.DoParamEvent(FOnMouseUp, v, FRunAlways);
end;
  ...
  if (Frame.Typ <> []) and (Frame.Color <> clNone) and (Frame.Width <> 0) then
    ...
    begin
      ...
      begin
        ...
        if (Pen.Width = 2) and (Frame.Style <> fsSolid) then
          d := 1 else
          d := 0;
      ...
      begin
        ...
        if (Pen.Width = 1) and (Frame.Style <> fsSolid) then
          d := 1 else
          d := 0;
...
begin
  ...
  FDrawText.UseDefaulCharset := FUseDefaulCharset;
...
begin
  ...
  if FUseDefaulCharset <> m.UseDefaulCharset then
    Result := Result + ' UseDefaulCharset="' + frxValueToXML(UseDefaulCharset) + '"';
...
begin
  inherited;
  ThLocale := 0;
  LocCharset := ((Font.Charset <> DEFAULT_CHARSET) and  not FUseDefaulCharset);
  if IsDataField then
  begin
    if LocCharset then
    begin
...
{$ENDIF}
  ...
  try
    ...
    begin
    if (Font.Charset <> DEFAULT_CHARSET) and not FUseDefaulCharset then
...
begin
  DoModify(nil);
  Report.DoNotifyEvent(Sender, FOnActivate, FRunAlways);
...
begin
  Report.DoNotifyEvent(Sender, FOnClick, FRunAlways);
end;
...
begin
  ...
  Report.DoParamEvent(FOnCloseQuery, v, FRunAlways);
  CanClose := v[1];
end;

procedure TfrxDialogPage.DoOnDeactivate(Sender: TObject);
begin
  Report.DoNotifyEvent(Sender, FOnDeactivate, FRunAlways);
end;

procedure TfrxDialogPage.DoOnHide(Sender: TObject);
begin
  Report.DoNotifyEvent(Sender, FOnHide, FRunAlways);
end;
...
begin
  ...
  Report.DoNotifyEvent(Sender, FOnShow, FRunAlways);
end;

procedure TfrxDialogPage.DoOnResize(Sender: TObject);
begin
  Report.DoNotifyEvent(Sender, FOnResize, FRunAlways);
end;
...
var
  ...
  var
    ...
    s: String;
    i: Integer;
    cFrom, cTo, cToChild: TfrxComponent;
    ChildB1: TfrxChild;
    sBand: TfrxComponent;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        cTo.Create(ToParent);
        cTo.Name := cFrom.Name;
        {avoid problem with child bands}
        if cFrom is TfrxBand then
        begin
          ChildB1 := TfrxBand(cFrom).Child;
          sBand := cTo;
          {handling all child bands chain}
          while (ChildB1 <> nil) do
          begin
            cToChild := TfrxComponent(ChildB1.NewInstance);
            cToChild.Create(ToParent);
            if ToParent.Report.FindObject(ChildB1.Name) <> nil then
              cToChild.Name := ''
            else
              cToChild.Name := ChildB1.Name;
            TfrxBand(sBand).Child := TfrxChild(cToChild);
            EnumObjects(cToChild, ChildB1);
            sBand := ChildB1.Child;
            FromParent.Objects.Remove(ChildB1);
            ChildB1.Free;
            ChildB1 := sBand as TfrxChild;
            SBand := cToChild;
          end;
        end
        else if cFrom is TfrxSubreport then
        begin
  ...
  var
    dsIdx: Integer;
  begin
    for dsIdx := 0 to Self.DataSets.Count - 1 do
      if tempReport.DataSets.Find(Self.DataSets.Items[dsIdx].DataSetName) = nil then
        tempReport.DataSets.Add(Self.DataSets.Items[dsIdx].DataSet);
  end;
...
begin
  Result := TfrxView(Integer(FSelfValue.Value));
end;
...
begin
  FSelfValue.Value := Integer(Value);
end;

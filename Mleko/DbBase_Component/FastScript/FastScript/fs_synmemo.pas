...
{$IFNDEF FPC}
  Windows, Messages,
{$ELSE}
  LCLType, LMessages, Buttons, LCLIntf, LResources,
{$ENDIF}
...
{$IFDEF Delphi12}
    while CharInSet(s[FPos.X],WordChars)
      or ((s[FPos.X] >= Char($007F)) and (s[FPos.X] <= Char($FFFF))) do
{$ELSE}
...
begin
  ...
  Dec(i);
{$IFDEF Delphi12}
  while (i > 0) and not (CharInSet(s[i], WordChars) or ((s[i] >= Char($007F)) and (s[i] <= Char($FFFF)))) do
    if s[i] = Char(LineBreak[1]) then
{$ELSE}
...
{$ENDIF}
      ...
      Dec(i);
{$IFDEF Delphi12}
  while (i > 0) and (CharInSet(s[i], WordChars) or ((s[i] >= Char($007F)) and (s[i] <= Char($FFFF)))) do
{$ELSE}
...
begin
  ...
  i := GetPlainTextPos(FPos);
{$IFDEF Delphi12}
  while (i < Length(s)) and (CharInSet(s[i], WordChars)) do
    Inc(i);
  while (i < Length(s)) and not (CharInSet(s[i], WordChars)) do
    if s[i] = Char(LineBreak[1]) then
      break else
      Inc(i);
{$ELSE}
...
begin
  ...
  begin
{$IFDEF FPC}
    // temporary fix for gtk1. Will be removed as LCL be fixed

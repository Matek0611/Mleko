...
TQuantizeThread = class;


{ Method type used with MultiSaveToStream. ImageNo starts at value 1 }
TGetGraphicMethod = function( const ImageNo     : Integer;
                              var   FreeGraphic : Boolean ) : TGraphic of object;
...
protected
    ...
    FQuantizeThread        : TQuantizeThread;
    ...
    procedure SetPaletteHandle( const InPalette : HPalette );
    ...
    { MB Oct 1, 2003 }
    procedure AssignFrames( const Graphic : TDibGraphic );

    { MB Oct 1, 2003. Assign the graphic only, not the frames }
    procedure AssignGraphic( const Graphic : TDibGraphic );
  ...
  public
    ...
    { Save a multipage graphic file in a single operation without having
      to use multiple calls to AppendToStream. Some graphic formats are
      not efficient with AppendToStream, like TPdfGraphic, so this method
      is much faster with TPdfGraphic.
      OnGetGraphic will be called ImageCount times to retrieve the
      images to be saved. The default implementation is to use
      AppendToStream multiple times. }
    procedure MultiSaveToStream( const Stream       : TStream;
                                 const ImageCount   : Integer;
                                 const OnGetGraphic : TGetGraphicMethod ); virtual;

    procedure MultiSaveToFile( const FileName     : String;
                               const ImageCount   : Integer;
    ...
    { clear the graphic, does not clear the frames }
    procedure Clear;

    { MB Oct 1, 2003. Clear frames only }
    procedure ClearFrames;

    { MB Oct 8, 2004. Add a frame at the end of the frame list }
    procedure AddFrame( const Graphic : TDibGraphic );

    { Fill the entire graphic with Color }
    procedure Fill( const Color : TRgb );

    { MB Oct 9, 2003. Creates a palette in a background thread for a true color
      image that will be used if drawing on a palette video mode
      (ie, 256 or less colors). Should be called when the image changes.
      The resulting palette is put in FPaletteHandle }
    procedure CreateOptimizedPalette;

    { Since CreateOptimizedPalette is time consuming, DestroyOptimizedPalette
      should be called manually when the graphic changes, and a new display
      palette is required. }
    procedure DestroyOptimizedPalette;
...
TQuantizeThread = class(TThread)
    constructor Create( const InDibGraphic : TDibGraphic );
    procedure Execute; override;
protected
    FDibGraphic : TDibGraphic;
end;
...
{ set to true if video mode at start was 256 colors or less }
PaletteVideoMode : Boolean;

{--------------------------------------------------------------------------}

constructor TQuantizeThread.Create( const InDibGraphic : TDibGraphic );
begin
    inherited Create(True);
    Self.FreeOnTerminate := True;
    FDibGraphic := InDibGraphic;
end;
...
procedure TQuantizeThread.Execute;
var
    LastPercent : ShortInt;
begin
    GetOptimizedPalette( FDibGraphic,
                         FDibGraphic.FPalette,
                         256,
                         nil,
                         LastPercent );

    FDibGraphic.FPaletteHandle := CreatePalette(PLogPalette(@FDibGraphic.FPalette)^);
    FDibGraphic.Changed(Self);
end;
...
procedure TDibGraphic.ClearFrames;
var
    ImageNo : Integer;
begin
...
procedure TDibGraphic.AddFrame( const Graphic : TDibGraphic );
begin
    FFrames.Add(Graphic);
end;
...
procedure TDibGraphic.CreateOptimizedPalette;
begin
    if not PaletteVideoMode then
        Exit;
    ...
    if Self.ImageFormat <> ifTrueColor then
        { image is already palette based }
        Exit;

    if FQuantizeThread <> nil then
        Exit;

    FQuantizeThread := TQuantizeThread.Create(Self);
    FQuantizeThread.Resume;
end;

{--------------------------------------------------------------------------}

procedure TDibGraphic.DestroyOptimizedPalette;
begin
    PaletteHandle := 0;
end;
...
begin
    ...
    begin
        ...
        if Self.PaletteHandle = 0 then
            DCPalette := EnDiGrph.HalfTonePalette
        else
            DCPalette := Self.PaletteHandle;

        OldPalette := SelectPalette(ACanvas.Handle, DCPalette, False);
        RealizePalette(ACanvas.Handle);
    end;
...
procedure TDibGraphic.AssignFrames( const Graphic : TDibGraphic );
var
    FrameNo     : Integer;
...
begin
    ...
    for FrameNo := 1 to Graphic.FrameCount do
    begin
        TempGraphic := TDibGraphic.Create;
        TempGraphic.AssignGraphic(TDibGraphic(Graphic.Frames[FrameNo]));

        Self.FFrames.Add(TempGraphic);
    end;
end;

{--------------------------------------------------------------------------}

procedure TDibGraphic.AssignGraphic( const Graphic : TDibGraphic );
begin
    if (Graphic.Width > 0) and (Graphic.Height > 0) then
    begin
        Self.NewImage( Graphic.Width,
                       Graphic.Height,
                       Graphic.ImageFormat,
                       @Graphic.FPalette,
                       Graphic.XDotsPerInch,
                       Graphic.YDotsPerInch );

        Self.XDotsPerInch   := Graphic.XDotsPerInch;
        Self.YDotsPerInch   := Graphic.YDotsPerInch;

        Self.ImageDelay     := Graphic.ImageDelay;

        Move( Graphic.FBits^, Self.FBits^,
              Graphic.ScanLineSize * Graphic.Height);
...
begin
    ...
    else if (Source is TDibGraphic) then
    begin
        AssignGraphic(TDibGraphic(Source));
        AssignFrames(TDibGraphic(Source));
    end
    ...
    begin
        ...
        if (TBitmap(Source).PixelFormat in [pf8bit, pf4bit]) then
            Self.PaletteHandle := TBitmap(Source).Palette;
...
begin
    { MB Oct 9, 2003 A true color image may be assigned a palette using
      the TQuantizeThread }
    Result := FPaletteHandle;
    if Result <> 0 then
        Exit;

    { assert FPaletteHandle = 0 }
    if ImageFormat <> ifTrueColor then
    begin
...
procedure TDibGraphic.SetPaletteHandle( const InPalette : HPalette );
begin
    if FPaletteHandle <> 0 then
    begin
        ...
        if InPalette <> 0 then
            FPaletteHandle := CopyPalette(InPalette)
        else
            FPaletteHandle := 0;
    end;
...
procedure TDibGraphic.MultiSaveToStream( const Stream       : TStream;
                                         const ImageCount   : Integer;
                                         const OnGetGraphic : TGetGraphicMethod );
var
    ImageNo     : Integer;
...
begin
    for ImageNo := 1 to ImageCount do
    begin
        Graphic := OnGetGraphic(ImageNo, FreeGraphic);
        try
            Self.Assign(Graphic);
        finally
            if FreeGraphic then
...
procedure TDibGraphic.MultiSaveToFile( const FileName     : String;
                                       const ImageCount   : Integer;
                                       const OnGetGraphic : TGetGraphicMethod );
var
...
begin
    ...
    try
        MultiSaveToStream(FileStream, ImageCount, OnGetGraphic);
    finally
        FileStream.Free;
    end;
end;
...
function PaletteIsGray(Palette : TMaxLogPalette):Boolean;
var
    I : Integer;
begin
    Result:=True;
    for I := 0 to Palette.palNumEntries-1 do
    begin
       if (Palette.palPalEntry[i].peRed<>Palette.palPalEntry[i].peGreen) or
          (Palette.palPalEntry[i].peRed<>Palette.palPalEntry[i].peBlue) then
       begin
           Result:=False;
           Break;
       end;
...
begin
    ...
    try
        ...
        { FP Feb 19, 2004. Check for 256 gray palette }
        if (ImageFormat=ifColor256) and PaletteIsGray(Palette) then
            ImageFormat := ifGray256;

        { FP Feb 19, 2004. Check for 16 gray palette }
        if (ImageFormat=ifColor16) and PaletteIsGray(Palette) then
            ImageFormat := ifGray16;


        { Rex Bloom }
        FlipBits := False;
        if (ImageFormat = ifBlackWhite) and
             (Palette.palPalEntry[0].peRed <> 0) then
        begin
        ...
        begin
            ...
            begin
                ...
                for X := 0 to (Self.ScanLineSize-1) do
...
begin
    ...
    Self.AssignGraphic(TDibGraphic(FFrames.Items[FrameNo]));
...
initialization
    ...
    ScreenDC := GetDC(0);
    try
        PaletteVideoMode := ((GetDeviceCaps(ScreenDC, Windows.RASTERCAPS) and Windows.RC_PALETTE) <> 0);
    finally
        ReleaseDC(0, ScreenDC);
    end;

...
function DateToDateStr( const Date  : TDateTime;
                        const Mask  : AnsiString ) : AnsiString;
...
function ExecWait( const CmdLine  : String;
                   const Show     : Boolean;
...
var
    SavedStr : AnsiString;
begin
    SavedStr := AnsiString(ShortDateFormat);
    try
        ShortDateFormat := Mask;
        Result := AnsiString(DateToStr(Date));
    finally
...
begin
    ...
    SavedStr := AnsiString(LongTimeFormat);
    try
        LongTimeFormat := Mask;
        Result := AnsiString(TimeToStr(Time));
    finally
...
begin
    ...
    begin
        try
            ...
            begin
                Result := GetExitCodeProcess(ProcessInfo.hProcess, TempExitCode);
                if Result then

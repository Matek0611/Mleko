...
| Module: EnTrsOcr
|
| Description: Driver for Transym OCR engine.
|
...
begin
    ...
    {$Warnings Off}
    CheckOCRCall(OCRInitialise(@JobNo));
    {$Warnings On}

    try

        {$Warnings Off}
        CheckOCRCall(OCRDoJob(JobNo, @JobInfo));
        {$Warnings On}
        ...
        repeat
            ...
            {$Warnings Off}
                CheckOCRCall(OCRGetJobStatusEx(JobNo, @JobStatus, @Progress, @Orientation));
            {$Warnings On}
        ...
        begin
            ...
            {$Warnings Off}
                CheckOCRCall( OCRGetJobResults( JobNo, @ResultSize, nil ));
            {$Warnings On}
            ...
            begin
                ...
                try
                    ...
                    pOCRResults^.Header.NumItems := 0;
                        ...
                        CheckOCRCall( OCRGetJobResults( JobNo, @ResultSize, ResultsPtr ));
        ...
        if JobNo <> -1 then
            CheckOCRCall(OCRShutDown(JobNo));

...
public
    ...
    { MB Jan 22, 2005 FillOrder option. Related to tiff tag, 266, Fill Order. }
    property FillOrderLSB2MSB : Boolean read FFillOrderLSB2MSB
                                        write FFillOrderLSB2MSB;
...
begin
    ...
    begin
        ...
        if Tag.Write then
        begin
            TIFFSetPCharField( TiffHandle, CCustomTagValues[Index], PAnsiChar(Tag.Value));
        end;
    end;
    ...
    if TiffGraphic.FDocumentNameTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_DOCUMENTNAME, PAnsiChar(TiffGraphic.FDocumentNameTag));

    if TiffGraphic.FImageDescriptionTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_IMAGEDESCRIPTION, PAnsiChar(TiffGraphic.FImageDescriptionTag));

    if TiffGraphic.FScannerMakeTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_MAKE, PAnsiChar(TiffGraphic.FScannerMakeTag));

    if TiffGraphic.FScannerModelTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_MODEL, PAnsiChar(TiffGraphic.FScannerModelTag));

    if TiffGraphic.FPageNameTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_PAGENAME, PAnsiChar(TiffGraphic.FPageNameTag));

    if TiffGraphic.FSoftwareTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_SOFTWARE, PAnsiChar(TiffGraphic.FSoftwareTag));

    if TiffGraphic.FDateTimeTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_DATETIME, PAnsiChar(TiffGraphic.FDateTimeTag));

    if TiffGraphic.FArtistTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_ARTIST, PAnsiChar(TiffGraphic.FArtistTag));

    if TiffGraphic.FHostComputerTag <> '' then
        TIFFSetPCharField( TiffHandle, TIFFTAG_HOSTCOMPUTER, PAnsiChar(TiffGraphic.FHostComputerTag));
end;
...
begin
    ...
    try
        ...
        { MB May 13 2003 }
        if not TIFFGetField(TiffHandle, TIFFTAG_ORIENTATION, Orientation) then
        ...
        { MB May 18. Orientation is very seldomly used but
          ORIENTATION_BOTRIGHT has been encountered }
        if Orientation = ORIENTATION_BOTRIGHT then
        begin
            RotateTransform := TRotateTransform.Create;
            try
                RotateTransform.Angle := 180;
                RotateTransform.Apply(Dib);
            finally
...
begin
    ...
    { MB Mar 30, 2004 Default JPG quality not set }
    FJpegQuality  := 70;

    { never set to True by default as this will break tiff streams with
      PDF files, in EnPdfGr.Pas }
    FFillOrderLSB2MSB := False;
...
begin
    ...
    begin
        ...
        Self.FJpegQuality      := TTiffGraphic(Source).FJpegQuality;
        Self.FFillOrderLSB2MSB := TTiffGraphic(Source).FFillOrderLSB2MSB;
    end;
end;
...
begin
    { MB Jun 20, 2000. The upper bound is choosen arbitrarily. I have seen a file
      ...
      MB Feb 17. No reason to put an upper bound. The tiff spec does not have
      an upper bound. Also, TPdfGraphic can create tiff sub images with
      RowsPerStrip equal to the height of the file }
    FRowsPerStrip := BoundInteger(InRowsPerStrip, 1, MaxLongInt);
end;
...
begin
    case ImageFormat of
        ...
        ifBlackWhite :
        begin
            if not (InCompression in [ tcNone, tcPackbits, tcZLib,
        ...
        ifColor256 :
        begin
            if not (InCompression in [ tcNone, tcPackbits,
                                       tcZlib, tcLZW ]) then
                RaiseCompressionNotSupported(ImageFormat, InCompression);
        end;
    ...
    try
        ...
        case Dib.Compression of
            ...
            begin
                ...
                if Dib.Compression = tcGroup3_2d then
                    TIFFSetIntField(TiffHandle, TIFFTAG_GROUP3OPTIONS, 5) //Changed to effect byte aligned EOL's. -IR.
                else
                    TIFFSetIntField(TiffHandle, TIFFTAG_GROUP3OPTIONS, 4);
            end;
            tcGroup4     :
            begin
        ...
        if Dib.SoftwareTag = '' then
            TIFFSetPCharField( TiffHandle, TIFFTAG_SOFTWARE,
                               PAnsiChar('Envision Image Library'));
        ...
        //Added fields 04/28/04 -IR.
        TIFFSetIntField(TiffHandle, TIFFTAG_SUBFILETYPE, 2); //NewSubFileType
        TIFFSetIntField(TiffHandle, TIFFTAG_ORIENTATION	, ORIENTATION_TOPLEFT); //Orientation

        if Dib.FillOrderLSB2MSB then
            TIFFSetIntField(TiffHandle, TIFFTAG_FILLORDER, 2)
        else
            TIFFSetIntField(TiffHandle, TIFFTAG_FILLORDER, 1);

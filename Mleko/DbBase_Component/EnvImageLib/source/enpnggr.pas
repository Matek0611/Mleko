...
public
    ...
    { MB Oct 30. Override Assign to copy a few properties }
...
begin
    if not IgnorePngWarnings then
        ...
        Windows.MessageBox( 0, PWideChar(msg), 'PNG Error', MB_OK or MB_ICONWARNING);
        {$else}
        Windows.MessageBox( 0, PChar(msg), 'PNG Error', MB_OK or MB_ICONWARNING);
        {$endif}
end;
...
begin
    ...
    begin
        FImageGamma              := TPngGraphic(Source).FImageGamma;
        FBackgroundColor         := TPngGraphic(Source).FBackgroundColor;
        FTransparentColor        := TPngGraphic(Source).FTransparentColor;

        FOverwriteFileBackground := TPngGraphic(Source).FOverwriteFileBackground;

        FCompressionLevel        := TPngGraphic(Source).FCompressionLevel;
        FSaveProgressive         := TPngGraphic(Source).FSaveProgressive;
        FSaveWithBackground      := TPngGraphic(Source).FSaveWithBackground;
        FSaveTransparent         := TPngGraphic(Source).FSaveTransparent;

        { See comment with msgGrayScaleTransparentPngNotSupported }
        if not (ImageFormat in [ifBlackWhite, ifGray16, ifGray256]) then
            FSaveTransparent         := TPngGraphic(Source).FSaveTransparent;
    end;
...
begin
    ...
    png_ptr := png_create_read_struct( PAnsiChar(PNG_LIBPNG_VER_STRING), nil,
    ...
    try
        ...
        begin
            ...
            { MB Oct 29, 2003. If file has a background set the property
              when loading }
            if Bitdepth = 16 then
            begin
                FBackgroundColor.Red   := pBackground^.Red div 256;
                FBackgroundColor.Green := pBackground^.Green div 256;
                FBackgroundColor.Blue  := pBackground^.Blue div 256;
            end
            else
            begin
                FBackgroundColor.Red   := pBackground^.Red ;
                FBackgroundColor.Green := pBackground^.Green;
                FBackgroundColor.Blue  := pBackground^.Blue;
            end;

            FSaveWithBackground    := True;
            FSaveTransparent       := True;
...
begin
    ...
    png_ptr := png_create_write_struct( PAnsiChar(PNG_LIBPNG_VER_STRING), nil,
                                        png_error_proc, png_warn_proc );
    if png_ptr = nil then

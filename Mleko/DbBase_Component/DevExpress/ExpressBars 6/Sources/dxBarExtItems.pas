...
{$ENDIF}
  ...
  dxBarSkinConsts, cxControls, dxCore;
    ...
    case Key of
      ...
      VK_HOME:
        if Shift = [ssCtrl] then
          SelStart := SelStart - (GetDateElement(SelStart, deDay) - 1)
        else
...
var
  ...
  begin
    ...
    begin
      ...
      Brush.Color := Painter.DateNavigatorHeaderColor;
      with TextR do
        cxExtTextOut(Handle, S, Point((Left + Right - Size.cX) div 2, (Top + Bottom - Size.cY) div 2),
          TextR, ETO_CLIPPED or ETO_OPAQUE);
      ...
      begin
        ...
        GetTextExtentPoint32(Handle, PChar(S), Length(S), Size);
        with TextR do
          cxExtTextOut(Handle, S, Point(Right - 3 - Size.cX, (Top + Bottom - Size.cY) div 2),
            TextR, ETO_OPAQUE);
      end;
...
function TdxBarDateCombo.GetDateOfText(AText: string): TDateTime;
  ...
  begin
    Result := False;
    for I := 1 to 7 do
      if Pos(ShortDayNames[I] + ' ', S) <> 0 then
      begin
        Result := True;
        Break;
      end;
...
begin
  Result := cxGetStringAdjustedToWidth(0, 0, inherited GetDisplayText(AText), 300);
end;

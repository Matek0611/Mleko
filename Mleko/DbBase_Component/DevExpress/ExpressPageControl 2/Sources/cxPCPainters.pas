...
begin
  ...
  begin
    ...
    case ATab.PaintingPositionIndex of
      ...
      4:
        RotateBitmap(ABitmap, ra180, True);
      5, 10:
        RotateBitmap(ABitmap, ra0, True);
      6, 11:
        RotateBitmap(ABitmap, raPlus90, True);
      7:
        RotateBitmap(ABitmap, ra180);
      12:
        RotateBitmap(ABitmap, raMinus90, True);
    end;
  end;
end;

procedure TcxPCTabsPainter.PrepareTabBitmapBackground(ABitmap: TBitmap;
...
begin
  if IsNativePainting and ParentControl.ParentBackground then
    ...
    FillRectByColor(ABitmap.Canvas.Handle, Rect(0, 0, ABitmap.Width, ABitmap.Height),
      GetFreeSpaceColor);
end;
...
begin
  ...
  if Result.Right - Result.Left < AMinFrameRectSize then
    Result.Right := Result.Left + AMinFrameRectSize;
  if Result.Bottom - Result.Top < AMinFrameRectSize then
...
begin
  ...
  R := GetControlRect(ParentControl);
  if IsNativePainting and ParentControl.ParentBackground then
    cxDrawThemeParentBackground(ParentControl, ParentInfo.Canvas, R)
  else
    with ParentInfo.Canvas do
    begin

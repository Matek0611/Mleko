...
function TcxOLAPDataQueryBuilder.GetAllTuples(AFields: TcxPivotGridFields;
  ...
  begin
    ...
    for I := 0 to ATupleElements.Count - 1 do
      Result := Result + ATupleElements[I] + ', ';
    TrimTrailingComma(Result);
    Result := Result + ')';
  end;
...
begin
  ...
  try
    ...
    if ATupleElements.Count > 0 then
      Result := Result + GetTuple(ATupleElements) + ', ';
    ALevel := 0;
    for I := 0 to ATuples.Count - 1 do
    begin
...
begin
  ...
  begin
    for I := 0 to ATuples.Count - 1 do
      Result := Result + GetHierarchyExpand(AField, ATuples[I]) + ', ';
  end
  else
  begin
    for I := 0 to ATuples.Count - 1 do
      Result := Result + GetCrossJoinExpand(AField, ATuples[I]) + ', ';
  end;
  TrimTrailingComma(Result);
  Result := Result + ' } ';
end;
...
begin
  ...
  for I := 0 to AFilter.Count - 1 do
    Result := Result + CorrectMemberName(Member(AFilter[I]).UniqueName) + ', ';
  TrimTrailingComma(Result);
end;
...
begin
  ...
  begin
    ...
    for I := 0 to AFilters.Count - 1 do
    begin
      if (I + 1 < AFilters.Count) and (AFilters[I].Group = AFilters[I + 1].Group) then
        Continue;
      AList := TcxPivotGridOLAPField.PopulateFilteredValues(AFilters[I], True);
      try
        Result := Result + '{' + GetMembers(AList) + '} on ' + IntToStr(AxisIndex) + ', ';
        Inc(AxisIndex);
      finally
        AList.Free;
      end;
  ...
  begin
    ...
    begin
      if IncludeLastMember or (I > 0) then
        Result := ', ' + ATuple.UniqueName + Result;
      ATuple := ATuple.Parent;
      Inc(I);
    end;
    Delete(Result, 1, 2);
...
function TcxOLAPDataQueryBuilder.WriteSortedMembers(AField: TcxPivotGridField;
  AContent: string): string;
begin
  Result := 'order({' + AContent + '}, ' + GetSortBy(AField.Link) + ', ' +
    GetSortOrder(AField) + ')';
end;
...
begin
  ...
  case FVersion of
    ...
    9: Result := Result + '.currentmember.' + 'member_value';
  end;
end;
